...
@EUDFunc
def f_main(cp):
    # (Line 18) if (f.Yume_Voice[cp] > 0)
    if EUDIf()(f.Yume_Voice[cp] <= 0, neg=True):
        # (Line 19) {
        # (Line 20) Text(f.Yume_Voice[cp]);
        Text(f.Yume_Voice[cp])
        # (Line 21) looker[cp] = f.Yume_Voice[cp];
        _ARRW(looker, cp) << (f.Yume_Voice[cp])
        # (Line 22) f.Yume_Voice[cp] = 0;
...
def Text(num):
    ...
    # (Line 30) case 1:
    _t1 = EUDSwitchCase()
    # (Line 31) PlayWAV("Yume_Unique.ogg");
    if _t1(1):
        # (Line 32) f.stb.print("\x13\x1BTsubakuro Yume\n");
        DoActions(PlayWAV("Yume_Unique.ogg"))
        f.stb.print("\x13\x1BTsubakuro Yume\n")
        # (Line 33) f.stb.print("\x13\x04시간이 없으니까 이제... 끝낼게!\x04\n");
        f.stb.print("\x13\x04시간이 없으니까 이제... 끝낼게!\x04\n")
        # (Line 34) break;
        EUDBreak()
        # (Line 35) case 2:
    _t2 = EUDSwitchCase()
    # (Line 36) PlayWAV("Yume_01.ogg");
    if _t2(2):
        # (Line 37) f.stb.print("\x13\x1BTsubakuro Yume\n");
        DoActions(PlayWAV("Yume_01.ogg"))
        f.stb.print("\x13\x1BTsubakuro Yume\n")
        # (Line 38) f.stb.print("\x13\x04그렇게 나와야지, 그렇지 않으면 내가 굉장하다는 걸 보여줄 수 없잖아\x04\n");
        f.stb.print("\x13\x04그렇게 나와야지, 그렇지 않으면 내가 굉장하다는 걸 보여줄 수 없잖아\x04\n")
        # (Line 39) break;
        EUDBreak()
        # (Line 40) case 3:
    _t3 = EUDSwitchCase()
    # (Line 41) PlayWAV("Yume_02.ogg");
    if _t3(3):
        # (Line 42) f.stb.print("\x13\x1BTsubakuro Yume\n");
        DoActions(PlayWAV("Yume_02.ogg"))
        f.stb.print("\x13\x1BTsubakuro Yume\n")
        # (Line 43) f.stb.print("\x13\x04벌써 끝이야? 아직 아직, 이제부터지!\x04\n");
        f.stb.print("\x13\x04벌써 끝이야? 아직 아직, 이제부터지!\x04\n")
        # (Line 44) break;
        EUDBreak()
        # (Line 45) case 4:
    _t4 = EUDSwitchCase()
    # (Line 46) PlayWAV("Yume_03.ogg");
    if _t4(4):
        # (Line 47) f.stb.print("\x13\x1BTsubakuro Yume\n");
        DoActions(PlayWAV("Yume_03.ogg"))
        f.stb.print("\x13\x1BTsubakuro Yume\n")
        # (Line 48) f.stb.print("\x13\x04어이쿠~ 라거나!\x04\n");
        f.stb.print("\x13\x04어이쿠~ 라거나!\x04\n")
        # (Line 49) break;
        EUDBreak()
        # (Line 50) case 5:
    _t5 = EUDSwitchCase()
    # (Line 51) PlayWAV("Yume_Ult.ogg");
    if _t5(5):
        # (Line 52) break;
        ...
        # (Line 53) case 6:
    _t6 = EUDSwitchCase()
    # (Line 54) f.stb.print("\x13\x1BTsubakuro Yume\n");
    if _t6(6):
        f.stb.print("\x13\x1BTsubakuro Yume\n")
        # (Line 55) f.stb.print("\x13\x04아아~ 쓸 데 없는 시간 써버렸다\x04\n");
        f.stb.print("\x13\x04아아~ 쓸 데 없는 시간 써버렸다\x04\n")
        # (Line 56) break;
        EUDBreak()
        # (Line 57) case 7:
    _t7 = EUDSwitchCase()
    # (Line 58) PlayWAV("Yume_04.ogg");
    if _t7(7):
        # (Line 59) f.stb.print("\x13\x1BTsubakuro Yume\n");
        DoActions(PlayWAV("Yume_04.ogg"))
        f.stb.print("\x13\x1BTsubakuro Yume\n")
        # (Line 60) f.stb.print("\x13\x04닛카리 아오에\x04\n");
        f.stb.print("\x13\x04닛카리 아오에\x04\n")
        # (Line 61) break;
        EUDBreak()
        # (Line 62) case 8:
    _t8 = EUDSwitchCase()
    # (Line 63) PlayWAV("Yume_05.ogg");
    if _t8(8):
        # (Line 64) break;
