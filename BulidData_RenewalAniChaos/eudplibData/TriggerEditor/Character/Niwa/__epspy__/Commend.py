...
def f_main(cp):
    ...
    if EUDIf()(EUDSCAnd()(f.wait[cp] >= 2)(f.count[cp] == 0)()):
        ...
        _t9 = EUDElseIf()
        # (Line 52) && Bring(cp, AtLeast, 1, "Protoss Arbiter", "[Skill]UseSkill") && f.step[cp] == 200)
        if _t9(EUDSCAnd()(Bring(cp, AtLeast, 1, "Protoss Carrier", "[Skill]UseSkill"))(Bring(cp, AtLeast, 1, "Protoss Arbiter", "[Skill]UseSkill"))(f.step[cp] == 200)()):
            ...
            # (Line 61) }
            DoActions(KillUnitAt(1, "Protoss Arbiter", "[Skill]UseSkill", cp))
            # (Line 62) else if (Bring(cp, AtLeast, 3, "Protoss Carrier", "[Skill]UseSkill")
        _t10 = EUDElseIf()
        # (Line 63) && f.step[cp] == 200)
        if _t10(EUDSCAnd()(Bring(cp, AtLeast, 3, "Protoss Carrier", "[Skill]UseSkill"))(f.step[cp] == 200)()):
            # (Line 64) {
            # (Line 65) f.Voice_Routine(cp, 6);
            ...
            _ARRW(f.step, cp) << (210)
            # (Line 70) KillUnitAt(3, "Protoss Carrier", "[Skill]UseSkill", cp);
            # (Line 71) }
            DoActions(KillUnitAt(3, "Protoss Carrier", "[Skill]UseSkill", cp))
            # (Line 74) else if (Bring(cp, AtLeast, 2, "Protoss Arbiter", "[Skill]UseSkill")
        _t11 = EUDElseIf()
        # (Line 75) && f.step[cp] == 300
        # (Line 76) && Deaths(cp, AtLeast, f.UltimateA[cp], " `UltimateCoolTime"))
        if _t11(EUDSCAnd()(Bring(cp, AtLeast, 2, "Protoss Arbiter", "[Skill]UseSkill"))(f.step[cp] == 300)(Deaths(cp, AtLeast, f.UltimateA[cp], " `UltimateCoolTime"))()):
            # (Line 77) {
            # (Line 78) if (Switch("UiltimateSwitch", Cleared))
            if EUDIf()(Switch("UiltimateSwitch", Cleared)):
                # (Line 79) {
                # (Line 80) f.Voice_Routine(cp, 2);
                ...
                _ARRW(f.wait, cp) << (0)
                # (Line 84) f.count[cp] = 0;
                _ARRW(f.count, cp) << (0)
                # (Line 85) f.loop[cp] = 0;
                _ARRW(f.loop, cp) << (0)
                # (Line 86) f.step[cp] = 310;
                _ARRW(f.step, cp) << (310)
                # (Line 87) SetDeaths(cp, Subtract, f.UltimateA[cp], " `UltimateCoolTime");
                # (Line 88) KillUnitAt(2, "Protoss Arbiter", "[Skill]UseSkill", cp);
                ...
                # (Line 91) {
            if EUDElse()():
                # (Line 92) SetResources(CurrentPlayer, Add, 600, Gas);
                # (Line 93) KillUnitAt(2, "Protoss Arbiter", "[Skill]UseSkill", cp);
