...
def SetDeathValue(cp):
    ...
    if EUDIf()(Deaths(CurrentPlayer, (0), 1000, (210))):
        ...
        if EUDIf()(EUDSCAnd()(cp >= 3)(cp <= 5)()):
            ...
            EUDEndIf()
            if EUDIf()(Deaths(cp, 10, 22, 172)):
                # (Line 54) SetResources((0), (8), v.DeathOre[cp], (0));
                DoActions(SetDeaths(cp, SetTo, 0, " `UniqueSkill"))
            EUDEndIf()
            # (Line 55) SetResources((1), (8), v.DeathOre[cp], (0));
            DoActions(SetResources((0), (8), v.DeathOre[cp], (0)))
            # (Line 56) SetResources((2), (8), v.DeathOre[cp], (0));
            DoActions(SetResources((1), (8), v.DeathOre[cp], (0)))
            # (Line 57) }
            DoActions(SetResources((2), (8), v.DeathOre[cp], (0)))
            # (Line 59) v.DeathOre_Text[0] = v.DeathOre[cp];
        EUDEndIf()
        _ARRW(v.DeathOre_Text, 0) << (v.DeathOre[cp])
        # (Line 60) v.DeathOre_Text[1] = v.DeathOre[cp];
        _ARRW(v.DeathOre_Text, 1) << (v.DeathOre[cp])
        # (Line 61) v.DeathOre_Text[2] = v.DeathOre[cp];
        _ARRW(v.DeathOre_Text, 2) << (v.DeathOre[cp])
        # (Line 62) v.DeathOre_Text[3] = v.DeathOre[cp];
        _ARRW(v.DeathOre_Text, 3) << (v.DeathOre[cp])
        # (Line 63) v.DeathOre_Text[4] = v.DeathOre[cp];
        _ARRW(v.DeathOre_Text, 4) << (v.DeathOre[cp])
        # (Line 64) v.DeathOre_Text[5] = v.DeathOre[cp];
        _ARRW(v.DeathOre_Text, 5) << (v.DeathOre[cp])
        # (Line 66) v.Death_Score[cp] += 1;
        _ARRW(v.Death_Score, cp).__iadd__(1)
        # (Line 67) }
        # (Line 68) }
    EUDEndIf()
    # (Line 71) function DeathText(cp)

# (Line 72) {
@EUDFunc
def DeathText(cp):
    # (Line 73) if (v.DeathOre_Text[cp] != 0)
    if EUDIf()(v.DeathOre_Text[cp] == 0, neg=True):
        # (Line 74) {
        # (Line 75) const value = dwread_epd(EPD(0x58A364 + 48 * 211 + 4 * cp));
        value = f_dwread_epd(EPD(0x58A364 + 48 * 211 + 4 * cp))
        # (Line 77) f.stb.printAt(1,"\n\x13\x08!! Warning !!\n\x13\x19:: \x17",
        # (Line 78) ptr2s(v.Hero_Name[value]), "\x04이 치명상을 입었습니다. \x19::\n\n\x13\x07",
        # (Line 79) v.Team_Num[cp], "\x04Team \x17+ ",
        # (Line 80) v.DeathOre_Text[cp], " \x04Ore\n");
        f.stb.printAt(1, "\n\x13\x08!! Warning !!\n\x13\x19:: \x17", ptr2s(v.Hero_Name[value]), "\x04이 치명상을 입었습니다. \x19::\n\n\x13\x07", v.Team_Num[cp], "\x04Team \x17+ ", v.DeathOre_Text[cp], " \x04Ore\n")
        # (Line 82) SetDeaths((13), (7), 0, (211));
        # (Line 83) v.DeathOre_Text[cp] = 0;
        DoActions(SetDeaths((13), (7), 0, (211)))
        _ARRW(v.DeathOre_Text, cp) << (0)
        # (Line 84) }
        # (Line 85) }
    EUDEndIf()
    # (Line 87) function SetKillScore(cp)

# (Line 88) {
@EUDFunc
def SetKillScore(cp):
    # (Line 89) if (Kills(CurrentPlayer, AtLeast, 1, 51))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 51); }
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 51)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 51))
        # (Line 90) if (Kills(CurrentPlayer, AtLeast, 1, 77))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 77); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 77)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 77))
        # (Line 91) if (Kills(CurrentPlayer, AtLeast, 1, 74))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 74); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 74)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 74))
        # (Line 92) if (Kills(CurrentPlayer, AtLeast, 1, 16))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 16); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 16)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 16))
        # (Line 93) if (Kills(CurrentPlayer, AtLeast, 1, 54))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 54); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 54)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 54))
        # (Line 94) if (Kills(CurrentPlayer, AtLeast, 1, 99))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 99); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 99)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 99))
        # (Line 95) if (Kills(CurrentPlayer, AtLeast, 1, 64))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 64); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 64)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 64))
        # (Line 96) if (Kills(CurrentPlayer, AtLeast, 1, 53))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 53); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 53)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 53))
        # (Line 97) if (Kills(CurrentPlayer, AtLeast, 1, 34))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 34); }
    EUDEndIf()
    if EUDIf()(Kills(CurrentPlayer, AtLeast, 1, 34)):
        _ARRW(v.Kill_Score, cp).__iadd__(1)
        DoActions(SetKills(CurrentPlayer, Subtract, 1, 34))
        # (Line 98) if (Kills(CurrentPlayer, AtLeast, 1, 10))	{ v.Kill_Score[cp] += 1; SetKills(CurrentPlayer, Subtract, 1, 10); }
    EUDEndIf()
