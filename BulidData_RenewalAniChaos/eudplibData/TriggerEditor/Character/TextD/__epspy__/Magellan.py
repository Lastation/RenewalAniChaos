## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY
from eudplib import *

def _IGVA(vList, exprListGen):
    def _():
        exprList = exprListGen()
        SetVariables(vList, exprList)
    EUDOnStart(_)

def _CGFW(exprf, retn):
    rets = [ExprProxy(None) for _ in range(retn)]
    def _():
        vals = exprf()
        for ret, val in zip(rets, vals):
            ret._value = val
    EUDOnStart(_)
    return rets

def _ARR(items):
    k = EUDArray(len(items))
    for i, item in enumerate(items):
        k[i] = item
    return k

def _VARR(items):
    k = EUDVArray(len(items))()
    for i, item in enumerate(items):
        k[i] = item
    return k

def _SRET(v, klist):
    return List2Assignable([v[k] for k in klist])

def _SV(dL, sL):
    [d << s for d, s in zip(FlattenList(dL), FlattenList(sL))]

class _ATTW:
    def __init__(self, obj, attrName):
        self.obj = obj
        self.attrName = attrName

    def __lshift__(self, r):
        setattr(self.obj, self.attrName, r)

    def __iadd__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov + v)

    def __isub__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov - v)

    def __imul__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov * v)

    def __ifloordiv__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov // v)

    def __iand__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov & v)

    def __ior__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov | v)

    def __ixor__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov ^ v)

class _ARRW:
    def __init__(self, obj, index):
        self.obj = obj
        self.index = index

    def __lshift__(self, r):
        self.obj[self.index] = r

    def __iadd__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov + v

    def __isub__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov - v

    def __imul__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov * v

    def __ifloordiv__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov // v

    def __iand__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov & v

    def __ior__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov | v

    def __ixor__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov ^ v

def _L2V(l):
    ret = EUDVariable()
    if EUDIf()(l):
        ret << 1
    if EUDElse()():
        ret << 0
    EUDEndIf()
    return ret

def _MVAR(vs):
    return List2Assignable([
        v.makeL() if IsEUDVariable(v) else EUDVariable() << v
        for v in FlattenList(vs)])

def _LSH(l, r):
    if IsEUDVariable(l):  return f_bitlshift(l, r)
    else: return l << r

## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY

# (Line 1) import PluginVariables as msqcvar;
import PluginVariables as msqcvar
# (Line 2) import Variable as v;
import Variable as v
# (Line 3) import Function as f;
import Function as f
# (Line 5) function NormalText(cp)
# (Line 6) {
@EUDFunc
def NormalText(cp):
    # (Line 7) switch (v.Sound_Text4[cp])
    EUDSwitch(v.Sound_Text4[cp])
    # (Line 8) {
    # (Line 9) case 3000:
    _t1 = EUDSwitchCase()
    # (Line 10) PlayWAV("Magellan_02.ogg");
    if _t1(3000):
        # (Line 11) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04업데이트 완료\n\n");
        DoActions(PlayWAV("Magellan_02.ogg"))
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04업데이트 완료\n\n")
        # (Line 12) v.Sound_Text4[cp] = 0;
        _ARRW(v.Sound_Text4, cp) << (0)
        # (Line 13) break;
        EUDBreak()
        # (Line 14) case 3001:
    _t2 = EUDSwitchCase()
    # (Line 15) PlayWAV("Magellan_01.ogg");
    if _t2(3001):
        # (Line 16) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04효율 개선!\n\n");
        DoActions(PlayWAV("Magellan_01.ogg"))
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04효율 개선!\n\n")
        # (Line 17) v.Sound_Text4[cp] = 0;
        _ARRW(v.Sound_Text4, cp) << (0)
        # (Line 18) break;
        EUDBreak()
        # (Line 19) case 3002:
    _t3 = EUDSwitchCase()
    # (Line 20) PlayWAV("Magellan_03.ogg");
    if _t3(3002):
        # (Line 21) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04일 시작!\n\n");
        DoActions(PlayWAV("Magellan_03.ogg"))
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04일 시작!\n\n")
        # (Line 22) v.Sound_Text4[cp] = 0;
        _ARRW(v.Sound_Text4, cp) << (0)
        # (Line 23) break;
        EUDBreak()
        # (Line 24) case 3003:
    _t4 = EUDSwitchCase()
    # (Line 25) PlayWAV("Magellan_04.ogg");
    if _t4(3003):
        # (Line 26) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04시스템 올 그린!\n\n");
        DoActions(PlayWAV("Magellan_04.ogg"))
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04시스템 올 그린!\n\n")
        # (Line 27) v.Sound_Text4[cp] = 0;
        _ARRW(v.Sound_Text4, cp) << (0)
        # (Line 28) break;
        EUDBreak()
        # (Line 29) case 3004:
    _t5 = EUDSwitchCase()
    # (Line 30) PlayWAV("Magellan_05.ogg");
    if _t5(3004):
        # (Line 31) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04가라! 눈의 반사로 눈속임이야!\n\n");
        DoActions(PlayWAV("Magellan_05.ogg"))
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04가라! 눈의 반사로 눈속임이야!\n\n")
        # (Line 32) v.Sound_Text4[cp] = 0;
        _ARRW(v.Sound_Text4, cp) << (0)
        # (Line 33) break;
        EUDBreak()
        # (Line 35) }
    # (Line 37) }
    EUDEndSwitch()
    # (Line 39) function UniqueText(cp)

# (Line 40) {
@EUDFunc
def UniqueText(cp):
    # (Line 41) switch (v.Sound_Text_Uniq[cp])
    EUDSwitch(v.Sound_Text_Uniq[cp])
    # (Line 42) {
    # (Line 43) case 21000:
    _t1 = EUDSwitchCase()
    # (Line 44) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04좋아, 드론 기동!\n\n");
    if _t1(21000):
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04좋아, 드론 기동!\n\n")
        # (Line 45) PlayWAV("Magellan_O.ogg");
        # (Line 46) v.Sound_Text_Uniq[cp] = 0;
        DoActions(PlayWAV("Magellan_O.ogg"))
        _ARRW(v.Sound_Text_Uniq, cp) << (0)
        # (Line 47) break;
        EUDBreak()
        # (Line 48) }
    # (Line 49) }
    EUDEndSwitch()
    # (Line 51) function UiltimateText(cp)

# (Line 52) {
@EUDFunc
def UiltimateText(cp):
    # (Line 53) switch (v.Sound_Text_Uilti[cp])
    EUDSwitch(v.Sound_Text_Uilti[cp])
    # (Line 54) {
    # (Line 55) case 21000:
    _t1 = EUDSwitchCase()
    # (Line 56) f.stb.print("\n\n\x13\x19Magellan\n\x13\x043... 2... 1...! 가라!\n\n");
    if _t1(21000):
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x043... 2... 1...! 가라!\n\n")
        # (Line 57) PlayWAV("Magellan_Ult.ogg");
        # (Line 58) v.Sound_Text_Uilti[cp] = 0;
        DoActions(PlayWAV("Magellan_Ult.ogg"))
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 59) break;
        EUDBreak()
        # (Line 60) case 21001:
    _t2 = EUDSwitchCase()
    # (Line 61) f.stb.print("\n\n\x13\x19Magellan\n\x13\x04꽃들아, 피어나라!\n\n");
    if _t2(21001):
        f.stb.print("\n\n\x13\x19Magellan\n\x13\x04꽃들아, 피어나라!\n\n")
        # (Line 62) PlayWAV("Magellan_Ult2.ogg");
        # (Line 63) v.Sound_Text_Uilti[cp] = 0;
        DoActions(PlayWAV("Magellan_Ult2.ogg"))
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 64) break;
        EUDBreak()
        # (Line 66) }
    # (Line 68) }
    EUDEndSwitch()
