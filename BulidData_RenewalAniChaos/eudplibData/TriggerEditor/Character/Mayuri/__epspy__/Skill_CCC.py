...
def f_main(cp):
    ...
    if EUDIf()(f.delay[cp] == 0):
        ...
        if EUDElseIf()(f.count[cp] == 2):
            # (Line 123) {
            # (Line 124) if (Bring(cp, AtLeast, 1, "Protoss Scout", "[Skill]UseSkill")
            _t17 = EUDIf()
            # (Line 125) && Bring(cp, AtLeast, 1, "Protoss Arbiter", "[Skill]UseSkill")
            # (Line 126) && f.step[cp] == 210)
            if _t17(EUDSCAnd()(Bring(cp, AtLeast, 1, "Protoss Scout", "[Skill]UseSkill"))(Bring(cp, AtLeast, 1, "Protoss Arbiter", "[Skill]UseSkill"))(f.step[cp] == 210)()):
                # (Line 127) {
                # (Line 128) f.Voice_Routine(cp, 7);
                f.Voice_Routine(cp, 7)
                # (Line 129) f.wait[cp] = 0;
                _ARRW(f.wait, cp) << (0)
                # (Line 130) f.count[cp] = 0;
                _ARRW(f.count, cp) << (0)
                # (Line 131) f.loop[cp] = 0;
                _ARRW(f.loop, cp) << (0)
                # (Line 132) f.step[cp] = 220;
                _ARRW(f.step, cp) << (220)
                # (Line 133) KillUnitAt(1, "Protoss Arbiter", "[Skill]UseSkill", cp);
                # (Line 134) KillUnitAt(1, "Protoss Scout", "[Skill]UseSkill", cp);
    ...
    if EUDIf()(f.delayB[cp] == 0):
        # (Line 151) {
        # (Line 152) if ((f.count[cp] == 1) && (f.loop[cp] < 16))
        if EUDIf()(EUDSCAnd()((f.count[cp] == 1))((EUDNot(f.loop[cp] >= 16)))()):
            # (Line 153) {
            # (Line 154) if (f.loopB[cp] > 3) f.loopB[cp] = 0;
            if EUDIf()(f.loopB[cp] <= 3, neg=True):
                _ARRW(f.loopB, cp) << (0)
                # (Line 156) f.Table_Sin(cp, (45 * f.loopB[cp] + 22), 50 * f.loopB[cp] + 50);
            EUDEndIf()
            f.Table_Sin(cp, (45 * f.loopB[cp] + 22), 50 * f.loopB[cp] + 50)
            # (Line 157) f.Table_Cos(cp, (45 * f.loopB[cp] + 22), 50 * f.loopB[cp] + 50);
            f.Table_Cos(cp, (45 * f.loopB[cp] + 22), 50 * f.loopB[cp] + 50)
            # (Line 159) f.SquareShape(cp, 1, " Unit. Hoffnung 25000", f.CosAngle[cp], f.SinAngle[cp]);
            f.SquareShape(cp, 1, " Unit. Hoffnung 25000", f.CosAngle[cp], f.SinAngle[cp])
            # (Line 161) KillUnitAt(All, " Unit. Hoffnung 25000", "Anywhere", cp);
            # (Line 163) f.SkillWaitB(cp, 160);
            ...
            # (Line 167) }
        EUDEndIf()
        # (Line 168) if (f.delayC[cp] == 0)
    EUDEndIf()
    if EUDIf()(f.delayC[cp] == 0):
        # (Line 169) {
        # (Line 170) if ((f.count[cp] < 2) && (f.loop[cp] < 16))
        if EUDIf()(EUDSCAnd()((EUDNot(f.count[cp] >= 2)))((EUDNot(f.loop[cp] >= 16)))()):
            # (Line 171) {
            # (Line 172) if (f.loopC[cp] > 8) f.loopC[cp] = 0;
            if EUDIf()(f.loopC[cp] <= 8, neg=True):
                _ARRW(f.loopC, cp) << (0)
                # (Line 174) f.SquareShape(cp, 1, "40 + 1n Zealot", 240 - 30 * f.loopC[cp], 30 * f.loopC[cp]);
            EUDEndIf()
            f.SquareShape(cp, 1, "40 + 1n Zealot", 240 - 30 * f.loopC[cp], 30 * f.loopC[cp])
            # (Line 175) f.SquareShape(cp, 1, "Protoss Dark Templar", 160 - 20 * f.loopC[cp], 20 * f.loopC[cp]);
            f.SquareShape(cp, 1, "Protoss Dark Templar", 160 - 20 * f.loopC[cp], 20 * f.loopC[cp])
            # (Line 177) KillUnitAt(All, "40 + 1n Zealot", "Anywhere", cp);
            # (Line 178) KillUnitAt(All, "Protoss Dark Templar", "Anywhere", cp);
            DoActions(KillUnitAt(All, "40 + 1n Zealot", "Anywhere", cp))
            # (Line 180) f.SkillWaitC(cp, 80);
            DoActions(KillUnitAt(All, "Protoss Dark Templar", "Anywhere", cp))
            f.SkillWaitC(cp, 80)
            # (Line 182) f.loopC[cp] += 1;
