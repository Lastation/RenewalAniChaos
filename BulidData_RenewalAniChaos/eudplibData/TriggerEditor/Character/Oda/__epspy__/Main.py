...
# (Line 3) import Character.Oda.Skill_A as A;
from Character.Oda import Skill_A as A
# (Line 4) import Character.Oda.Skill_S as S;
from Character.Oda import Skill_S as S
# (Line 5) import Character.Oda.Skill_C as C;
from Character.Oda import Skill_C as C
# (Line 6) import Character.Oda.Skill_O as O;
from Character.Oda import Skill_O as O
# (Line 8) import Character.Oda.Skill_SSS as SSS;
from Character.Oda import Skill_SSS as SSS
# (Line 9) import Character.Oda.Skill_SSSCC as SSSCC;
from Character.Oda import Skill_SSSCC as SSSCC
# (Line 10) import Character.Oda.Skill_SSSCCAA as SSSCCAA;
from Character.Oda import Skill_SSSCCAA as SSSCCAA
# (Line 12) import Character.Oda.Skill_CC as CC;
from Character.Oda import Skill_CC as CC
# (Line 13) import Character.Oda.Skill_CCA as CCA;
from Character.Oda import Skill_CCA as CCA
# (Line 14) import Character.Oda.Skill_CCAAA as CCAAA;
from Character.Oda import Skill_CCAAA as CCAAA
# (Line 15) import Character.Oda.Skill_CCAAAAA as CCAAAAA;
from Character.Oda import Skill_CCAAAAA as CCAAAAA
# (Line 17) import Character.Oda.Skill_AASS as AASS;
from Character.Oda import Skill_AASS as AASS
# (Line 19) import Character.Oda.Skill_AAAA as AAAA;
from Character.Oda import Skill_AAAA as AAAA
# (Line 20) import Character.Oda.Skill_AAAAAA as AAAAAA;
from Character.Oda import Skill_AAAAAA as AAAAAA
# (Line 21) import Character.Oda.Skill_AO as AO;
from Character.Oda import Skill_AO as AO
# (Line 23) import Character.Oda.Text as text;
from Character.Oda import Text as text
# (Line 24) import Character.Oda.Commend as commend;
from Character.Oda import Commend as commend
# (Line 26) var loop  = 0;
loop = EUDCreateVariables(1)
_IGVA([loop], lambda: [0])
# (Line 28) function SkillList(cp);
# (Line 30) function main(cp)
# (Line 31) {
@EUDFunc
def f_main(cp):
    # (Line 32) f.location[cp] = 193;
    _ARRW(f.location, cp) << (193)
    # (Line 33) f.heroID[cp] = 99;
    _ARRW(f.heroID, cp) << (99)
    # (Line 35) f.UltimateA[cp] = 650;
    _ARRW(f.UltimateA, cp) << (650)
    # (Line 36) f.UltimateB[cp] = 300;
    _ARRW(f.UltimateB, cp) << (300)
    # (Line 38) SkillList(cp);
    SkillList(cp)
    # (Line 39) commend.main(cp);
    commend.f_main(cp)
    # (Line 41) if (f.wait[cp] == 0)
    if EUDIf()(f.wait[cp] == 0):
        # (Line 42) {
        # (Line 43) switch(f.step[cp])
        EUDSwitch(f.step[cp])
        # (Line 44) {
        # (Line 45) case 1:
        _t2 = EUDSwitchCase()
    ...
    if EUDIf()(loop <= 12, neg=True):
        ...
        # (Line 100) }
    EUDEndIf()
    # (Line 103) function SkillVoice(cp)

# (Line 104) {
@EUDFunc
def SkillVoice(cp):
    # (Line 105) if (f.Oda_Voice[cp] != 0) { text.main(cp); }
    if EUDIf()(f.Oda_Voice[cp] == 0, neg=True):
        text.f_main(cp)
        # (Line 106) }
    EUDEndIf()
    # (Line 108) function SkillList(cp)

# (Line 109) {
@EUDFunc
def SkillList(cp):
    # (Line 110) if(Memory(0x00596A44, Exactly, 256)) 	// Insert key Pressed
    if EUDIf()(Memory(0x00596A44, Exactly, 256)):
        # (Line 111) {
        # (Line 112) f.stb.printAt(0, "\n");
        f.stb.printAt(0, "\n")
        # (Line 113) f.stb.printAt(1, "\x1F　＃\x1B- 오다 사쿠노스케　\x04[ 문호 스트레이독스 ]");
        f.stb.printAt(1, "\x1F　＃\x1B- 오다 사쿠노스케　\x04[ 문호 스트레이독스 ]")
        # (Line 114) f.stb.printAt(2, "　　\x1FA\x04ction List");
        ...
        # (Line 122) f.stb.printAt(10, "\n");
        f.stb.printAt(10, "\n")
        # (Line 123) PlayWAV("sound\\Bullet\\LaserHit.wav");
        # (Line 124) }
