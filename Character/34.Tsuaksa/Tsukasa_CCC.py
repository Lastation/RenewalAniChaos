...
function main(playerID)
{
   ...
   if (v.P_WaitMain[playerID] == 0)
   {
      ...
      else if (v.P_CountMain[playerID] == 1)
      {
         ...
         {
            for (i = 0; i < 3; i++) {
               trg.Table_Sin(playerID, 30 * i * v.P_LoopMain[playerID], d);
               trg.Table_Cos(playerID, 30 * i * v.P_LoopMain[playerID], d);
         ...
         {          
            for (i = 0; i < 3; i++) {
               trg.Table_Sin(playerID, 30 * i * v.P_LoopMain[playerID], 200);
               trg.Table_Cos(playerID, 30 * i * v.P_LoopMain[playerID], 200);
      ...
      else if (v.P_CountMain[playerID] == 2)
      {
         ...
         if (v.P_LoopMain[playerID] == 0) { x = 0; y = 2; }
         else if (v.P_LoopMain[playerID] == 1) { x = 1; y = 3; }
         else if (v.P_LoopMain[playerID] == 2) { x = 2; y = 3; }
         else if (v.P_LoopMain[playerID] == 3) { x = 3; y = 2; }
         else if (v.P_LoopMain[playerID] == 4) { x = 3; y = 1; }
         else if (v.P_LoopMain[playerID] == 5) { x = 2; y = 0; }
         else if (v.P_LoopMain[playerID] == 6) { x = 1; y = -1; }
         else if (v.P_LoopMain[playerID] == 7) { x = 0; y = -2; }

         if (v.P_LoopMain[playerID] < 8) {
            epic.Shape_Dot(playerID, 1, "60 + 1n Danimoth", t * x, t * y);
