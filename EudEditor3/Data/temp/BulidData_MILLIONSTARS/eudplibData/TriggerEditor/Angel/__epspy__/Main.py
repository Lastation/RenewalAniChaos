...
# (Line 2) import Angel.ButtonText as bt;
from Angel import ButtonText as bt
# (Line 3) import Angel.Effect as et;
from Angel import Effect as et
...
def Init_Weapon():
    ...
    if EUDWhile()(i >= 13, neg=True):
        def _t2():
            i.__iadd__(1)
        # (Line 51) {
        # (Line 52) w.Set_Label(v.Unit_AtkID[i], 229 + i);
        w.Set_Label(v.Unit_AtkID[i], 229 + i)
...
def Set_AtkPercent(type, value):
    ...
    # (Line 210) case 0:
    _t1 = EUDSwitchCase()
    # (Line 211) SetMemory(0x515BA0, SetTo, value);	// 소형 - 프린세스 	75%
    if _t1(0):
        # (Line 212) v.Unit_AtkPercent[0] = (value * 100)/256;
        DoActions(SetMemory(0x515BA0, SetTo, value))
        _ARRW(v.Unit_AtkPercent, 0) << ((value * 100) // 256)
        # (Line 213) break;
        EUDBreak()
        # (Line 214) case 1:
    _t2 = EUDSwitchCase()
    # (Line 215) SetMemory(0x515BA4, SetTo, value);	// 중형 - 페어리 		100%
    if _t2(1):
        # (Line 216) v.Unit_AtkPercent[1] = (value * 100)/256;
        DoActions(SetMemory(0x515BA4, SetTo, value))
        _ARRW(v.Unit_AtkPercent, 1) << ((value * 100) // 256)
        # (Line 217) break;
        EUDBreak()
        # (Line 218) case 2:
    _t3 = EUDSwitchCase()
    # (Line 219) SetMemory(0x515BA8, SetTo, value);	// 대형 - 엔젤 		50%
    if _t3(2):
        # (Line 220) v.Unit_AtkPercent[2] = (value * 100)/256;
        DoActions(SetMemory(0x515BA8, SetTo, value))
