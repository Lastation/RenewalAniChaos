## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY
from eudplib import *

def _IGVA(vList, exprListGen):
    def _():
        exprList = exprListGen()
        SetVariables(vList, exprList)
    EUDOnStart(_)

def _CGFW(exprf, retn):
    rets = [ExprProxy(None) for _ in range(retn)]
    def _():
        vals = exprf()
        for ret, val in zip(rets, vals):
            ret._value = val
    EUDOnStart(_)
    return rets

def _ARR(items):
    k = EUDArray(len(items))
    for i, item in enumerate(items):
        k[i] = item
    return k

def _VARR(items):
    k = EUDVArray(len(items))()
    for i, item in enumerate(items):
        k[i] = item
    return k

def _SRET(v, klist):
    return List2Assignable([v[k] for k in klist])

def _SV(dL, sL):
    [d << s for d, s in zip(FlattenList(dL), FlattenList(sL))]

class _ATTW:
    def __init__(self, obj, attrName):
        self.obj = obj
        self.attrName = attrName

    def __lshift__(self, r):
        setattr(self.obj, self.attrName, r)

    def __iadd__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov + v)

    def __isub__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov - v)

    def __imul__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov * v)

    def __ifloordiv__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov // v)

    def __iand__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov & v)

    def __ior__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov | v)

    def __ixor__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov ^ v)

class _ARRW:
    def __init__(self, obj, index):
        self.obj = obj
        self.index = index

    def __lshift__(self, r):
        self.obj[self.index] = r

    def __iadd__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov + v

    def __isub__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov - v

    def __imul__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov * v

    def __ifloordiv__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov // v

    def __iand__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov & v

    def __ior__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov | v

    def __ixor__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov ^ v

def _L2V(l):
    ret = EUDVariable()
    if EUDIf()(l):
        ret << 1
    if EUDElse()():
        ret << 0
    EUDEndIf()
    return ret

def _MVAR(vs):
    return List2Assignable([
        v.makeL() if IsEUDVariable(v) else EUDVariable() << v
        for v in FlattenList(vs)])

def _LSH(l, r):
    if IsEUDVariable(l):  return f_bitlshift(l, r)
    else: return l << r

## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY

# (Line 1) import Variable as v;
import Variable as v
# (Line 3) const s = StringBuffer();
s = _CGFW(lambda: [StringBuffer()], 1)[0]
# (Line 5) function NormalText(cp)
# (Line 6) {
@EUDFunc
def NormalText(cp):
    # (Line 7) switch (v.Sound_Text2[cp])
    EUDSwitch(v.Sound_Text2[cp])
    # (Line 8) {
    # (Line 9) case 2000:
    _t1 = EUDSwitchCase()
    # (Line 10) PlayWAV("Anzu_01.ogg");
    if _t1(2000):
        # (Line 11) v.Sound_Text2[cp] = 0;
        DoActions(PlayWAV("Anzu_01.ogg"))
        _ARRW(v.Sound_Text2, cp) << (0)
        # (Line 12) s.print("\n\x13\x1BFutaba Anzu\n\x13\x04엣헴!\x04\n");
        s.print("\n\x13\x1BFutaba Anzu\n\x13\x04엣헴!\x04\n")
        # (Line 13) break;
        EUDBreak()
        # (Line 14) case 2010:
    _t2 = EUDSwitchCase()
    # (Line 15) PlayWAV("Anzu_02.ogg");
    if _t2(2010):
        # (Line 16) v.Sound_Text2[cp] = 0;
        DoActions(PlayWAV("Anzu_02.ogg"))
        _ARRW(v.Sound_Text2, cp) << (0)
        # (Line 17) s.print("\n\x13\x1BFutaba Anzu\n\x13\x04지지 않으려면 ··· \x17이것 \x04밖에 없어!\n");
        s.print("\n\x13\x1BFutaba Anzu\n\x13\x04지지 않으려면 ··· \x17이것 \x04밖에 없어!\n")
        # (Line 18) break;
        EUDBreak()
        # (Line 19) case 2011:
    _t3 = EUDSwitchCase()
    # (Line 20) PlayWAV("Anzu_03.ogg");
    if _t3(2011):
        # (Line 21) v.Sound_Text2[cp] = 0;
        DoActions(PlayWAV("Anzu_03.ogg"))
        _ARRW(v.Sound_Text2, cp) << (0)
        # (Line 22) s.print("\n\x13\x1BFutaba Anzu\n\x13\x04우리의 \x17정의\x04를 위해 ―――――――!!\n");
        s.print("\n\x13\x1BFutaba Anzu\n\x13\x04우리의 \x17정의\x04를 위해 ―――――――!!\n")
        # (Line 23) break;
        EUDBreak()
        # (Line 24) }
    # (Line 25) }
    EUDEndSwitch()
    # (Line 27) function UiltimateText(cp)

# (Line 28) {
@EUDFunc
def UiltimateText(cp):
    # (Line 29) switch (v.Sound_Text_Uilti[cp])
    EUDSwitch(v.Sound_Text_Uilti[cp])
    # (Line 30) {
    # (Line 31) case 8000:
    _t1 = EUDSwitchCase()
    # (Line 32) PlayWAV("Anzu_Uiltimate.ogg");
    if _t1(8000):
        # (Line 33) v.Sound_Text_Uilti[cp] = 0;
        DoActions(PlayWAV("Anzu_Uiltimate.ogg"))
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 34) s.printAt(3,"\x13\x1BFutaba Anzu\n\x13\x17「 \x04일하지 않는 \x17모든 자\x04들에게 전한다 \x17」");
        s.printAt(3, "\x13\x1BFutaba Anzu\n\x13\x17「 \x04일하지 않는 \x17모든 자\x04들에게 전한다 \x17」")
        # (Line 35) break;
        EUDBreak()
        # (Line 36) case 8001:
    _t2 = EUDSwitchCase()
    # (Line 37) v.Sound_Text_Uilti[cp] = 0;
    if _t2(8001):
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 38) s.printAt(3,"\x13\x1BFutaba Anzu\n\x13\x17「 \x04이것은 \x17놀이\x04도 \x17라이브\x04도 아니야! \x17」");
        s.printAt(3, "\x13\x1BFutaba Anzu\n\x13\x17「 \x04이것은 \x17놀이\x04도 \x17라이브\x04도 아니야! \x17」")
        # (Line 39) break;
        EUDBreak()
        # (Line 40) case 8002:
    _t3 = EUDSwitchCase()
    # (Line 41) v.Sound_Text_Uilti[cp] = 0;
    if _t3(8002):
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 42) s.printAt(3,"\x13\x1BFutaba Anzu\n\x13\x17「 \x04우리의 \x17정의\x04를 위하여―――――――――!! \x17」");
        s.printAt(3, "\x13\x1BFutaba Anzu\n\x13\x17「 \x04우리의 \x17정의\x04를 위하여―――――――――!! \x17」")
        # (Line 43) break;
        EUDBreak()
        # (Line 44) case 8100:
    _t4 = EUDSwitchCase()
    # (Line 45) PlayWAV("Anzu_01.ogg");
    if _t4(8100):
        # (Line 46) v.Sound_Text_Uilti[cp] = 0;
        DoActions(PlayWAV("Anzu_01.ogg"))
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 47) s.printAt(3,"\x13\x1BFutaba Anzu\n\x13\x17「 \x04엣헴! \x17」");
        s.printAt(3, "\x13\x1BFutaba Anzu\n\x13\x17「 \x04엣헴! \x17」")
        # (Line 48) break;
        EUDBreak()
        # (Line 49) case 8101:
    _t5 = EUDSwitchCase()
    # (Line 50) PlayWAV("Anzu_Uiltimate2.ogg");
    if _t5(8101):
        # (Line 51) v.Sound_Text_Uilti[cp] = 0;
        DoActions(PlayWAV("Anzu_Uiltimate2.ogg"))
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 52) break;
        EUDBreak()
        # (Line 53) case 8102:
    _t6 = EUDSwitchCase()
    # (Line 54) v.Sound_Text_Uilti[cp] = 0;
    if _t6(8102):
        _ARRW(v.Sound_Text_Uilti, cp) << (0)
        # (Line 55) s.printAt(3,"\x13\x1BFutaba Anzu\n\x13\x17「 \x04˙˙˙ 라는 \x17꿈\x04을 꿨어 \x17」");
        s.printAt(3, "\x13\x1BFutaba Anzu\n\x13\x17「 \x04˙˙˙ 라는 \x17꿈\x04을 꿨어 \x17」")
        # (Line 56) break;
        EUDBreak()
        # (Line 57) }
    # (Line 58) }
    EUDEndSwitch()
    # (Line 60) function UniqueText(cp)

# (Line 61) {
@EUDFunc
def UniqueText(cp):
    # (Line 62) switch (v.Sound_Text_Uniq[cp])
    EUDSwitch(v.Sound_Text_Uniq[cp])
    # (Line 63) {
    # (Line 64) case 8000:
    _t1 = EUDSwitchCase()
    # (Line 65) PlayWAV("Anzu_Unique.ogg");
    if _t1(8000):
        # (Line 66) v.Sound_Text_Uniq[cp] = 0;
        DoActions(PlayWAV("Anzu_Unique.ogg"))
        _ARRW(v.Sound_Text_Uniq, cp) << (0)
        # (Line 67) s.printAt(3,"\x13\x1BFutaba Anzu\n\x13\x04「　시..싫어, 나는 \x17일\x04하지 않을꺼얏 !!　\x04」");
        s.printAt(3, "\x13\x1BFutaba Anzu\n\x13\x04「　시..싫어, 나는 \x17일\x04하지 않을꺼얏 !!　\x04」")
        # (Line 68) break;
        EUDBreak()
        # (Line 69) }
    # (Line 70) }
    EUDEndSwitch()
