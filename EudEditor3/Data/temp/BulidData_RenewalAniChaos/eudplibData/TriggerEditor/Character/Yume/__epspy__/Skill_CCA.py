...
@EUDFunc
def f_main(cp):
    # (Line 5) MoveLocation("26.Yume_Bozo", f.heroID[cp], cp, "Anywhere");
    ...
    if EUDIf()(f.delay[cp] == 0):
        ...
        if EUDIf()(f.count[cp] == 0):
            ...
            if EUDIf()(f.loop[cp] == 0):
                # (Line 13) {
                # (Line 14) SetSwitch("Recall - Yume", Set);
                # (Line 15) SetDeaths(cp, SetTo, 1, " `ShieldRecharge");
                DoActions(SetSwitch("Recall - Yume", Set))
                # (Line 17) f.SkillWait(cp, 400);
                DoActions(SetDeaths(cp, SetTo, 1, " `ShieldRecharge"))
                f.SkillWait(cp, 400)
                # (Line 19) f.loop[cp] += 1;
                _ARRW(f.loop, cp).__iadd__(1)
                # (Line 20) }
                # (Line 21) else if (f.loop[cp] < 21)
            if EUDElseIf()(f.loop[cp] >= 21, neg=True):
                # (Line 22) {
                # (Line 23) var deg = (dwrand() % 16) * 22;
                deg = EUDVariable()
                deg << ((f_dwrand() % 16) * 22)
                # (Line 24) var distance = (dwrand() % 3 + 7) * 10;
                distance = EUDVariable()
                distance << ((f_dwrand() % 3 + 7) * 10)
                # (Line 26) if (f.loop[cp] % 2 == 0)
                if EUDIf()(f.loop[cp] % 2 == 0):
                    # (Line 27) {
                    # (Line 28) RemoveUnitAt(All, "40 + 1n Wraith", "Anywhere", cp);
                    # (Line 30) f.LineShape(cp, 1, "40 + 1n Wraith", deg, 9, 75, distance);
                    DoActions(RemoveUnitAt(All, "40 + 1n Wraith", "Anywhere", cp))
                    f.LineShape(cp, 1, "40 + 1n Wraith", deg, 9, 75, distance)
                    # (Line 31) f.LineShape(cp, 1, "60 + 1n High Templar", deg, 9, 75, distance);
                    f.LineShape(cp, 1, "60 + 1n High Templar", deg, 9, 75, distance)
                    # (Line 32) }
                    # (Line 34) else if (f.loop[cp] % 2 == 1)
                if EUDElseIf()(f.loop[cp] % 2 == 1):
                    # (Line 35) {
                    # (Line 36) RemoveUnitAt(All, " Creep. Dunkelheit", "Anywhere", cp);
                    # (Line 38) f.LineShape(cp, 1, " Creep. Dunkelheit", deg, 9, 75, distance);
                    DoActions(RemoveUnitAt(All, " Creep. Dunkelheit", "Anywhere", cp))
                    f.LineShape(cp, 1, " Creep. Dunkelheit", deg, 9, 75, distance)
                    # (Line 39) f.LineShape(cp, 1, "Target", deg, 9, 75, distance);
                    f.LineShape(cp, 1, "Target", deg, 9, 75, distance)
                    # (Line 40) }
                    # (Line 42) deg = (dwrand() % 16) * 22;
                EUDEndIf()
                deg << ((f_dwrand() % 16) * 22)
