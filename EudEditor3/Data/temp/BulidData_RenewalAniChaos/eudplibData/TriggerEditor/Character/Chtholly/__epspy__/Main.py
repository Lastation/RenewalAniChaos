...
# (Line 11) import Character.Chtholly.Skill_CCACA as CCACA;
from Character.Chtholly import Skill_CCACA as CCACA
# (Line 13) import Character.Chtholly.Text as text;
from Character.Chtholly import Text as text
# (Line 14) import Character.Chtholly.Commend as commend;
...
@EUDFunc
def f_main(cp):
    # (Line 20) f.location[cp] = 117;
    _ARRW(f.location, cp) << (117)
    # (Line 21) f.heroID[cp] = 77;
    _ARRW(f.heroID, cp) << (77)
    # (Line 23) f.UltimateA[cp] = 700;
    _ARRW(f.UltimateA, cp) << (700)
    # (Line 24) f.UltimateB[cp] = 700;
    ...
    if EUDIf()(f.wait[cp] == 0):
        ...
        # (Line 58) }
    EUDEndIf()
    # (Line 61) function SkillVoice(cp)

# (Line 62) {
@EUDFunc
def SkillVoice(cp):
    # (Line 63) if (f.Chtholly_Voice[cp] != 0) { text.main(cp); }
    if EUDIf()(f.Chtholly_Voice[cp] == 0, neg=True):
        text.f_main(cp)
        # (Line 64) }
    EUDEndIf()
    # (Line 66) function SkillList(cp)

# (Line 67) {
@EUDFunc
def SkillList(cp):
    # (Line 68) if(Memory(0x00596A44, Exactly, 256)) 	// Insert key Pressed
    if EUDIf()(Memory(0x00596A44, Exactly, 256)):
        # (Line 69) {
        # (Line 70) f.stb.printAt(0, "\n");
        f.stb.printAt(0, "\n")
        # (Line 71) f.stb.printAt(1, "\x1F　＃\x04- 크톨리 \x1B노타 세니오리스　\x04[ 종말에 뭐 하세요? 바쁘세요? 구해 주실 수 있나요? ]");
        f.stb.printAt(1, "\x1F　＃\x04- 크톨리 \x1B노타 세니오리스　\x04[ 종말에 뭐 하세요? 바쁘세요? 구해 주실 수 있나요? ]")
        # (Line 72) f.stb.printAt(2, "　　\x1FA\x04ction List\r\n　　　\x07O \x19 전투속행 ON/OFF [ 침식 도중 사망시 5초간 전투 속행 ] \x05개인쿨 4분");
        f.stb.printAt(2, "　　\x1FA\x04ction List\r\n　　　\x07O \x19 전투속행 ON/OFF [ 침식 도중 사망시 5초간 전투 속행 ] \x05개인쿨 4분")
        # (Line 73) f.stb.printAt(3, "　　　\x1F>SSS \x04어쩔수 없지;; \x19[ 공성 ]");
        f.stb.printAt(3, "　　　\x1F>SSS \x04어쩔수 없지;; \x19[ 공성 ]")
        # (Line 74) f.stb.printAt(4, "　　　\x1F>CCA + CA + AAA \x04침식 \x19[ 공성 / AAA - 자리고정 / 귀환봉인 ] \x1F[1]");
        f.stb.printAt(4, "　　　\x1F>CCA + CA + AAA \x04침식 \x19[ 공성 / AAA - 자리고정 / 귀환봉인 ] \x1F[1]")
        # (Line 75) f.stb.printAt(5, "\n");
        f.stb.printAt(5, "\n")
        # (Line 76) f.stb.printAt(6, "　　　\x1F[1] + \x08O \x04세상에서 가장 행복한 여자 \x19[ 공성 / 선딜 4.5초 / 장판형 / 쉴드 10고정 ] \x05", f.UltimateA[cp]);
        f.stb.printAt(6, "　　　\x1F[1] + \x08O \x04세상에서 가장 행복한 여자 \x19[ 공성 / 선딜 4.5초 / 장판형 / 쉴드 10고정 ] \x05", f.UltimateA[cp])
        # (Line 77) f.stb.printAt(7, "　　　\x08AAAA \x04세상에서 가장 행복한 여자 \x19[ 공성 / 선딜 4.5초 / 장판형 / 쉴드 10고정 ] \x05", f.UltimateB[cp]);
        f.stb.printAt(7, "　　　\x08AAAA \x04세상에서 가장 행복한 여자 \x19[ 공성 / 선딜 4.5초 / 장판형 / 쉴드 10고정 ] \x05", f.UltimateB[cp])
        # (Line 78) f.stb.printAt(8, "　　　\x04 * \x19AAAA\x04사용시 \x19O\x04 사용시보다 데미지가 많이 약해집니다.");
        f.stb.printAt(8, "　　　\x04 * \x19AAAA\x04사용시 \x19O\x04 사용시보다 데미지가 많이 약해집니다.")
        # (Line 79) f.stb.printAt(9, "\n");
        f.stb.printAt(9, "\n")
