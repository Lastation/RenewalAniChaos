## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY
from eudplib import *

def _IGVA(vList, exprListGen):
    def _():
        exprList = exprListGen()
        SetVariables(vList, exprList)
    EUDOnStart(_)

def _CGFW(exprf, retn):
    rets = [ExprProxy(None) for _ in range(retn)]
    def _():
        vals = exprf()
        for ret, val in zip(rets, vals):
            ret._value = val
    EUDOnStart(_)
    return rets

def _ARR(items):
    k = EUDArray(len(items))
    for i, item in enumerate(items):
        k[i] = item
    return k

def _VARR(items):
    k = EUDVArray(len(items))()
    for i, item in enumerate(items):
        k[i] = item
    return k

def _SRET(v, klist):
    return List2Assignable([v[k] for k in klist])

def _SV(dL, sL):
    [d << s for d, s in zip(FlattenList(dL), FlattenList(sL))]

class _ATTW:
    def __init__(self, obj, attrName):
        self.obj = obj
        self.attrName = attrName

    def __lshift__(self, r):
        setattr(self.obj, self.attrName, r)

    def __iadd__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov + v)

    def __isub__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov - v)

    def __imul__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov * v)

    def __ifloordiv__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov // v)

    def __iand__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov & v)

    def __ior__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov | v)

    def __ixor__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov ^ v)

class _ARRW:
    def __init__(self, obj, index):
        self.obj = obj
        self.index = index

    def __lshift__(self, r):
        self.obj[self.index] = r

    def __iadd__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov + v

    def __isub__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov - v

    def __imul__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov * v

    def __ifloordiv__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov // v

    def __iand__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov & v

    def __ior__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov | v

    def __ixor__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov ^ v

def _L2V(l):
    ret = EUDVariable()
    if EUDIf()(l):
        ret << 1
    if EUDElse()():
        ret << 0
    EUDEndIf()
    return ret

def _MVAR(vs):
    return List2Assignable([
        v.makeL() if IsEUDVariable(v) else EUDVariable() << v
        for v in FlattenList(vs)])

def _LSH(l, r):
    if IsEUDVariable(l):  return f_bitlshift(l, r)
    else: return l << r

## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY

# (Line 1) import PluginVariables as msqcvar;
import PluginVariables as msqcvar
# (Line 2) import Function as f;
import Function as f
# (Line 4) function main(cp)
# (Line 5) {
@EUDFunc
def f_main(cp):
    # (Line 6) if (f.delay[cp] == 0)
    if EUDIf()(f.delay[cp] == 0):
        # (Line 7) {
        # (Line 8) if (f.count[cp] == 0)
        if EUDIf()(f.count[cp] == 0):
            # (Line 9) {
            # (Line 10) if (f.loop[cp] < 4)
            if EUDIf()(f.loop[cp] >= 4, neg=True):
                # (Line 11) {
                # (Line 12) f.SquareShape(cp, 1, "60 + 1n Archon", 200 - f.loop[cp] * 50, 0);
                f.SquareShape(cp, 1, "60 + 1n Archon", 200 - f.loop[cp] * 50, 0)
                # (Line 13) KillUnitAt(All, "60 + 1n Archon", "Anywhere", cp);
                # (Line 15) f.SkillWait(cp, 50);
                DoActions(KillUnitAt(All, "60 + 1n Archon", "Anywhere", cp))
                f.SkillWait(cp, 50)
                # (Line 16) f.loop[cp] += 1;
                _ARRW(f.loop, cp).__iadd__(1)
                # (Line 17) }
                # (Line 18) else if (f.loop[cp] == 4)
            if EUDElseIf()(f.loop[cp] == 4):
                # (Line 19) {
                # (Line 20) f.SquareShape(cp, 4, "60 + 1n Archon", 50, 50);
                f.SquareShape(cp, 4, "60 + 1n Archon", 50, 50)
                # (Line 21) KillUnitAt(All, "60 + 1n Archon", "Anywhere", cp);
                # (Line 23) f.SkillWait(cp, 50);
                DoActions(KillUnitAt(All, "60 + 1n Archon", "Anywhere", cp))
                f.SkillWait(cp, 50)
                # (Line 24) f.loop[cp] += 1;
                _ARRW(f.loop, cp).__iadd__(1)
                # (Line 25) }
                # (Line 26) else if (f.loop[cp] == 5)
            if EUDElseIf()(f.loop[cp] == 5):
                # (Line 27) {
                # (Line 28) f.NxNSquareShape(cp, 1, "50 + 1n Battlecruiser", 3, 75);
                f.NxNSquareShape(cp, 1, "50 + 1n Battlecruiser", 3, 75)
                # (Line 29) Order("50 + 1n Battlecruiser", cp, "Anywhere", Attack, "Anywhere");
                # (Line 31) f.SkillWait(cp, 0);
                DoActions(Order("50 + 1n Battlecruiser", cp, "Anywhere", Attack, "Anywhere"))
                f.SkillWait(cp, 0)
                # (Line 33) f.count[cp] += 1;
                _ARRW(f.count, cp).__iadd__(1)
                # (Line 34) f.loop[cp] = 0;
                _ARRW(f.loop, cp) << (0)
                # (Line 35) }
                # (Line 37) }
            EUDEndIf()
            # (Line 38) else if (f.count[cp] == 1)
        if EUDElseIf()(f.count[cp] == 1):
            # (Line 39) {
            # (Line 40) if (f.loop[cp] < 4)
            if EUDIf()(f.loop[cp] >= 4, neg=True):
                # (Line 41) {
                # (Line 42) f.distance[cp] = 125;
                _ARRW(f.distance, cp) << (125)
                # (Line 44) f.Table_Cos(cp, 45 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Cos(cp, 45 + 90 * f.loop[cp], f.distance[cp])
                # (Line 45) f.Table_Sin(cp, 45 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Sin(cp, 45 + 90 * f.loop[cp], f.distance[cp])
                # (Line 47) f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp]);
                f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp])
                # (Line 48) f.SkillUnit(cp, 1, "40 + 1n Wraith");
                f.SkillUnit(cp, 1, "40 + 1n Wraith")
                # (Line 49) f.SkillUnit(cp, 1, "40 + 1n Goliath");
                f.SkillUnit(cp, 1, "40 + 1n Goliath")
                # (Line 51) f.distance[cp] = 175;
                _ARRW(f.distance, cp) << (175)
                # (Line 53) f.Table_Cos(cp, 45 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Cos(cp, 45 + 90 * f.loop[cp], f.distance[cp])
                # (Line 54) f.Table_Sin(cp, 45 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Sin(cp, 45 + 90 * f.loop[cp], f.distance[cp])
                # (Line 56) f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp]);
                f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp])
                # (Line 57) f.SkillUnit(cp, 1, "40 + 1n Wraith");
                f.SkillUnit(cp, 1, "40 + 1n Wraith")
                # (Line 58) f.SkillUnit(cp, 1, "40 + 1n Goliath");
                f.SkillUnit(cp, 1, "40 + 1n Goliath")
                # (Line 60) KillUnitAt(All, "40 + 1n Wraith", "Anywhere", cp);
                # (Line 62) f.SkillWait(cp, 50);
                DoActions(KillUnitAt(All, "40 + 1n Wraith", "Anywhere", cp))
                f.SkillWait(cp, 50)
                # (Line 63) f.loop[cp] += 1;
                _ARRW(f.loop, cp).__iadd__(1)
                # (Line 64) }
                # (Line 65) else if (f.loop[cp] == 4)
            if EUDElseIf()(f.loop[cp] == 4):
                # (Line 66) {
                # (Line 67) MoveLocation(f.location[cp], f.heroID[cp], cp, "Anywhere");
                # (Line 68) KillUnitAt(All, "40 + 1n Goliath", "Anywhere", cp);
                DoActions(MoveLocation(f.location[cp], f.heroID[cp], cp, "Anywhere"))
                # (Line 70) f.SkillWait(cp, 0);
                DoActions(KillUnitAt(All, "40 + 1n Goliath", "Anywhere", cp))
                f.SkillWait(cp, 0)
                # (Line 72) f.count[cp] += 1;
                _ARRW(f.count, cp).__iadd__(1)
                # (Line 73) f.loop[cp] = 0;
                _ARRW(f.loop, cp) << (0)
                # (Line 74) }
                # (Line 75) }
            EUDEndIf()
            # (Line 76) else if (f.count[cp] == 2)
        if EUDElseIf()(f.count[cp] == 2):
            # (Line 77) {
            # (Line 78) if (f.loop[cp] < 4)
            if EUDIf()(f.loop[cp] >= 4, neg=True):
                # (Line 79) {
                # (Line 80) f.distance[cp] = 100;
                _ARRW(f.distance, cp) << (100)
                # (Line 82) f.Table_Cos(cp, 0 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Cos(cp, 0 + 90 * f.loop[cp], f.distance[cp])
                # (Line 83) f.Table_Sin(cp, 0 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Sin(cp, 0 + 90 * f.loop[cp], f.distance[cp])
                # (Line 85) f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp]);
                f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp])
                # (Line 86) f.SkillUnit(cp, 1, "40 + 1n Wraith");
                f.SkillUnit(cp, 1, "40 + 1n Wraith")
                # (Line 87) f.SkillUnit(cp, 1, "40 + 1n Goliath");
                f.SkillUnit(cp, 1, "40 + 1n Goliath")
                # (Line 89) f.distance[cp] = 150;
                _ARRW(f.distance, cp) << (150)
                # (Line 91) f.Table_Cos(cp, 0 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Cos(cp, 0 + 90 * f.loop[cp], f.distance[cp])
                # (Line 92) f.Table_Sin(cp, 0 + 90 * f.loop[cp], f.distance[cp]);
                f.Table_Sin(cp, 0 + 90 * f.loop[cp], f.distance[cp])
                # (Line 94) f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp]);
                f.MoveLoc(f.heroID[cp], cp, f.CosAngle[cp], f.SinAngle[cp])
                # (Line 95) f.SkillUnit(cp, 1, "40 + 1n Wraith");
                f.SkillUnit(cp, 1, "40 + 1n Wraith")
                # (Line 96) f.SkillUnit(cp, 1, "40 + 1n Goliath");
                f.SkillUnit(cp, 1, "40 + 1n Goliath")
                # (Line 98) KillUnitAt(All, "40 + 1n Wraith", "Anywhere", cp);
                # (Line 100) f.SkillWait(cp, 50);
                DoActions(KillUnitAt(All, "40 + 1n Wraith", "Anywhere", cp))
                f.SkillWait(cp, 50)
                # (Line 101) f.loop[cp] += 1;
                _ARRW(f.loop, cp).__iadd__(1)
                # (Line 102) }
                # (Line 103) else if (f.loop[cp] == 4)
            if EUDElseIf()(f.loop[cp] == 4):
                # (Line 104) {
                # (Line 105) MoveLocation(f.location[cp], f.heroID[cp], cp, "Anywhere");
                # (Line 106) MoveUnit(All, "40 + 1n Goliath", cp, "[Skill]Unit_Wait_ALL", f.location[cp]);
                DoActions(MoveLocation(f.location[cp], f.heroID[cp], cp, "Anywhere"))
                # (Line 107) Order("40 + 1n Goliath", cp, "Anywhere", Attack, f.location[cp]);
                DoActions(MoveUnit(All, "40 + 1n Goliath", cp, "[Skill]Unit_Wait_ALL", f.location[cp]))
                # (Line 109) f.SkillWait(cp, 50);
                DoActions(Order("40 + 1n Goliath", cp, "Anywhere", Attack, f.location[cp]))
                f.SkillWait(cp, 50)
                # (Line 111) f.count[cp] += 1;
                _ARRW(f.count, cp).__iadd__(1)
                # (Line 112) f.loop[cp] = 0;
                _ARRW(f.loop, cp) << (0)
                # (Line 113) }
                # (Line 114) }
            EUDEndIf()
            # (Line 115) else if (f.count[cp] == 3)
        if EUDElseIf()(f.count[cp] == 3):
            # (Line 116) {
            # (Line 117) KillUnitAt(All, "40 + 1n Goliath", "Anywhere", cp);
            # (Line 118) KillUnitAt(All, "50 + 1n Battlecruiser", "Anywhere", cp);
            DoActions(KillUnitAt(All, "40 + 1n Goliath", "Anywhere", cp))
            # (Line 119) f.SkillEnd(cp);
            DoActions(KillUnitAt(All, "50 + 1n Battlecruiser", "Anywhere", cp))
            f.SkillEnd(cp)
            # (Line 120) }
            # (Line 121) }
        EUDEndIf()
        # (Line 122) }
    EUDEndIf()
