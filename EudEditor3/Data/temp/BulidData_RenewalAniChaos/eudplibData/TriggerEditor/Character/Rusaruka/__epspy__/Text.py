...
@EUDFunc
def f_main(cp):
    # (Line 17) if (f.Rusaruka_Voice[cp] > 0)
    if EUDIf()(f.Rusaruka_Voice[cp] <= 0, neg=True):
        # (Line 18) {
        # (Line 19) Text(f.Rusaruka_Voice[cp]);
        Text(f.Rusaruka_Voice[cp])
        # (Line 20) looker[cp] = f.Rusaruka_Voice[cp];
        _ARRW(looker, cp) << (f.Rusaruka_Voice[cp])
        # (Line 21) f.Rusaruka_Voice[cp] = 0;
...
def Text(num):
    ...
    # (Line 29) case 1:
    _t1 = EUDSwitchCase()
    # (Line 30) PlayWAV("Rusaruka_1.ogg");
    if _t1(1):
        # (Line 31) f.stb.print("\x13\x1BRusaruka Schwagerin\n");
        DoActions(PlayWAV("Rusaruka_1.ogg"))
        f.stb.print("\x13\x1BRusaruka Schwagerin\n")
        # (Line 32) f.stb.print("\x13\x04[ \x06너무 얕보면... 먹어버릴거야? \x04]");
        f.stb.print("\x13\x04[ \x06너무 얕보면... 먹어버릴거야? \x04]")
        # (Line 33) break;
        EUDBreak()
        # (Line 34) case 2:
    _t2 = EUDSwitchCase()
    # (Line 35) PlayWAV("Rusaruka_2.ogg");
    if _t2(2):
        # (Line 36) f.stb.print("\x13\x1BRusaruka Schwagerin\n");
        DoActions(PlayWAV("Rusaruka_2.ogg"))
        f.stb.print("\x13\x1BRusaruka Schwagerin\n")
        # (Line 37) f.stb.print("\x13\x04[ \x06좋아, 내가 시험해줄게. 술래잡기를 계속하도록 할까? \x04]");
        f.stb.print("\x13\x04[ \x06좋아, 내가 시험해줄게. 술래잡기를 계속하도록 할까? \x04]")
        # (Line 38) break;
        EUDBreak()
        # (Line 39) case 3:
    _t3 = EUDSwitchCase()
    # (Line 40) PlayWAV("Rusaruka_3.ogg");
    if _t3(3):
        # (Line 41) f.stb.print("\x13\x1BRusaruka Schwagerin\n");
        DoActions(PlayWAV("Rusaruka_3.ogg"))
        f.stb.print("\x13\x1BRusaruka Schwagerin\n")
        # (Line 42) f.stb.print("\x13\x04[ \x06조금 아프겠지만, 참으렴...! \x04]\n\n");
        f.stb.print("\x13\x04[ \x06조금 아프겠지만, 참으렴...! \x04]\n\n")
        # (Line 43) break;
        EUDBreak()
        # (Line 44) case 4:
    _t4 = EUDSwitchCase()
    # (Line 45) PlayWAV("Rusaruka_4.ogg");
    if _t4(4):
        # (Line 46) f.stb.print("\x13\x1BRusaruka Schwagerin\n");
        DoActions(PlayWAV("Rusaruka_4.ogg"))
        f.stb.print("\x13\x1BRusaruka Schwagerin\n")
        # (Line 47) f.stb.print("\x13\x04[ 당신 여기서 \x06살해\x04당하고 싶어...? \x04]");
        f.stb.print("\x13\x04[ 당신 여기서 \x06살해\x04당하고 싶어...? \x04]")
        # (Line 48) break;
        EUDBreak()
        # (Line 49) case 5:
    _t5 = EUDSwitchCase()
    # (Line 50) PlayWAV("Rusalka_Ultimate.ogg");
    if _t5(5):
        # (Line 51) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
        DoActions(PlayWAV("Rusalka_Ultimate.ogg"))
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 52) f.stb.printAt(5, "\n\x13\x06무서웠단 말이야! 놔두고 가버릴지!\n\x13\x06싫단 말이야! 떠밀려버리면!\n\x13\x06나... 걷는 거 느리단 말이야─!");
        f.stb.printAt(5, "\n\x13\x06무서웠단 말이야! 놔두고 가버릴지!\n\x13\x06싫단 말이야! 떠밀려버리면!\n\x13\x06나... 걷는 거 느리단 말이야─!")
        # (Line 53) break;
        EUDBreak()
        # (Line 54) case 6:
    _t6 = EUDSwitchCase()
    # (Line 55) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
    if _t6(6):
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 56) f.stb.printAt(5, "\n\x13\x06따라 잡을 수 없다면,\n\x13\x06아예 멈춰 세우겠다고!\n\x13\x06그렇게 생각했어! 불만있어!?");
        f.stb.printAt(5, "\n\x13\x06따라 잡을 수 없다면,\n\x13\x06아예 멈춰 세우겠다고!\n\x13\x06그렇게 생각했어! 불만있어!?")
        # (Line 57) break;
        EUDBreak()
        # (Line 58) case 7:
    _t7 = EUDSwitchCase()
    # (Line 59) PlayWAV("Rusalka_Unique.ogg");
    if _t7(7):
        # (Line 60) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
        DoActions(PlayWAV("Rusalka_Unique.ogg"))
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 61) f.stb.printAt(5, "\n\x13\x05In der Nacht, wo alles schlaft\n\x13\x04모든것이 잠드는 밤에\n\n\x13\x05Wie schon, den Meeresboden zu verlassen.\n\x13\x04물밑을 벗어나는것이야 말로 아름다움이니.");
        f.stb.printAt(5, "\n\x13\x05In der Nacht, wo alles schlaft\n\x13\x04모든것이 잠드는 밤에\n\n\x13\x05Wie schon, den Meeresboden zu verlassen.\n\x13\x04물밑을 벗어나는것이야 말로 아름다움이니.")
        # (Line 62) break;
        EUDBreak()
        # (Line 63) case 8:
    _t8 = EUDSwitchCase()
    # (Line 64) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
    if _t8(8):
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 65) f.stb.printAt(5, "\n\x13\x05Ich hebe den Kopf uber das Wasser\n\x13\x04수면에 고개 들어\n\n\x13\x05Welch Freude, das Spiel der Wasserwellen\n\x13\x04물장구치며 놀자, 즐기어라.");
        f.stb.printAt(5, "\n\x13\x05Ich hebe den Kopf uber das Wasser\n\x13\x04수면에 고개 들어\n\n\x13\x05Welch Freude, das Spiel der Wasserwellen\n\x13\x04물장구치며 놀자, 즐기어라.")
        # (Line 66) break;
        EUDBreak()
        # (Line 67) case 9:
    _t9 = EUDSwitchCase()
    # (Line 68) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
    if _t9(9):
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 69) f.stb.printAt(5, "\n\x13\x05Durch die nun zerbrochene Stille\n\x13\x04맑은 대기를 울리고\n\n\x13\x05Rufen wir unsere Namen\n\x13\x04서로 크게 불러");
        f.stb.printAt(5, "\n\x13\x05Durch die nun zerbrochene Stille\n\x13\x04맑은 대기를 울리고\n\n\x13\x05Rufen wir unsere Namen\n\x13\x04서로 크게 불러")
        # (Line 70) break;
        EUDBreak()
        # (Line 71) case 10:
    _t10 = EUDSwitchCase()
    # (Line 72) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
    if _t10(10):
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 73) f.stb.printAt(5, "\n\x13\x05Pechschwarzes Haar wirbelt im Wind\n\x13\x04새까만 젖은 머리를 털어……\n\n\x13\x05Welch Freude, sie trocknen zu sehen\n\x13\x04말리며 놀자, 즐겨라");
        f.stb.printAt(5, "\n\x13\x05Pechschwarzes Haar wirbelt im Wind\n\x13\x04새까만 젖은 머리를 털어……\n\n\x13\x05Welch Freude, sie trocknen zu sehen\n\x13\x04말리며 놀자, 즐겨라")
        # (Line 74) break;
        EUDBreak()
        # (Line 75) case 11:
    _t11 = EUDSwitchCase()
    # (Line 76) f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n");
    if _t11(11):
        f.stb.printAt(3, "\x13\x1BRusaruka Schwagerin\n")
        # (Line 77) f.stb.printAt(5, "\n\x13\x05Briah――\n\x13\x04창조\n\n\x13\x05Csejte Ungarn Nachtzehrer\n\x13\x04체터성의 식인그림자\n\n");
        f.stb.printAt(5, "\n\x13\x05Briah――\n\x13\x04창조\n\n\x13\x05Csejte Ungarn Nachtzehrer\n\x13\x04체터성의 식인그림자\n\n")
        # (Line 78) break;
        EUDBreak()
        # (Line 79) }
