...
# (Line 13) function savePosRoutine(cp)
# (Line 14) {
@EUDFunc
def f_savePosRoutine(cp):
    ...
    if EUDWhile()(i <= 0, neg=True):
        ...
        # (Line 25) }
        # (Line 27) posX1[0] = dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20));
        EUDSetContinuePoint()
        _t2()
    EUDEndWhile()
    _ARRW(posX1, 0) << (f_dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20)))
    # (Line 28) posX2[0] = dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 8));
    _ARRW(posX2, 0) << (f_dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 8)))
    # (Line 29) posY1[0] = dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 4));
    _ARRW(posY1, 0) << (f_dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 4)))
    # (Line 30) posY2[0] = dwread_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 12));
    ...
    if EUDIf()(n >= max_n, neg=True):
        ...
        # (Line 33) }
    EUDEndIf()
    # (Line 35) function main(cp)

# (Line 36) {
@EUDFunc
def f_main(cp):
    # (Line 37) if (f.delay[cp] == 0)
    if EUDIf()(f.delay[cp] == 0):
        # (Line 38) {
        # (Line 39) if (f.count[cp] == 0)
        if EUDIf()(f.count[cp] == 0):
            ...
            if EUDIf()(f.loop[cp] == 1):
                ...
                if EUDIf()(n == max_n):
                    # (Line 50) {
                    # (Line 51) dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20), posX1[max_n]);
                    f_dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20), posX1[max_n])
                    # (Line 52) dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 8), posX2[max_n]);
                    f_dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 8), posX2[max_n])
                    # (Line 53) dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 4), posY1[max_n]);
                    f_dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 4), posY1[max_n])
                    # (Line 54) dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 12), posY2[max_n]);
                    f_dwwrite_epd(EPD(0x58DC60 + (f.location[cp] - 1) * 20 + 12), posY2[max_n])
                    # (Line 56) MoveUnit(All, f.heroID[cp], cp, "Anywhere", f.location[cp]);
                    # (Line 57) CenterView(f.location[cp]);
        ...
        if EUDElseIf()(f.count[cp] == 1):
            ...
            # (Line 74) SetDeaths(cp, SetTo, 2160, " `UniqueCoolTime");
            DoActions(SetSwitch("Recall - Oda", Clear))
            # (Line 75) f.SkillEnd(cp);
