...
def Shape_Square(playerID, count, unit, pos_x, pos_y):
    ...
    if EUDElse()():
        ...
        # (Line 359) }
    EUDEndIf()
    # (Line 381) function Shape_SquareWithAngle(playerID : TrgPlayer, count, unit : TrgUnit, distance, angle)

# (Line 382) {
@EUDTypedFunc([TrgPlayer, None, TrgUnit, None, None])
def Shape_SquareWithAngle(playerID, count, unit, distance, angle):
    # (Line 383) Table_Cos(playerID, angle, distance);
    Table_Cos(playerID, angle, distance)
    # (Line 384) Table_Sin(playerID, angle, distance);
    Table_Sin(playerID, angle, distance)
    # (Line 386) MoveLoc(v.P_UnitID[playerID], playerID, v.P_AngleCos[playerID] , v.P_AngleSin[playerID]);
    MoveLoc(v.P_UnitID[playerID], playerID, v.P_AngleCos[playerID], v.P_AngleSin[playerID])
    # (Line 387) SkillUnit(playerID, count, unit);
    SkillUnit(playerID, count, unit)
    # (Line 388) MoveLoc(v.P_UnitID[playerID], playerID, -v.P_AngleSin[playerID], v.P_AngleCos[playerID]);
    MoveLoc(v.P_UnitID[playerID], playerID, -v.P_AngleSin[playerID], v.P_AngleCos[playerID])
    # (Line 389) SkillUnit(playerID, count, unit);
    SkillUnit(playerID, count, unit)
    # (Line 390) MoveLoc(v.P_UnitID[playerID], playerID, -v.P_AngleCos[playerID], -v.P_AngleSin[playerID]);
    MoveLoc(v.P_UnitID[playerID], playerID, -v.P_AngleCos[playerID], -v.P_AngleSin[playerID])
    # (Line 391) SkillUnit(playerID, count, unit);
    SkillUnit(playerID, count, unit)
    # (Line 392) MoveLoc(v.P_UnitID[playerID], playerID, v.P_AngleSin[playerID], -v.P_AngleCos[playerID]);
    MoveLoc(v.P_UnitID[playerID], playerID, v.P_AngleSin[playerID], -v.P_AngleCos[playerID])
    # (Line 393) SkillUnit(playerID, count, unit);
    SkillUnit(playerID, count, unit)
    # (Line 394) }
    # (Line 415) function Shape_NxNSquare(playerID : TrgPlayer, count, unit : TrgUnit, size, interval)

# (Line 416) {
@EUDTypedFunc([TrgPlayer, None, TrgUnit, None, None])
def Shape_NxNSquare(playerID, count, unit, size, interval):
    # (Line 417) var loop = (size * size) / 4;
    loop = EUDVariable()
    loop << ((size * size) // 4)
    # (Line 418) var space = interval / 2;
    space = EUDVariable()
    space << (interval // 2)
    # (Line 419) var dis_x, dis_y;
    dis_x, dis_y = EUDCreateVariables(2)
    # (Line 421) if (size == 1)
    if EUDIf()(size == 1):
        # (Line 422) {
        # (Line 423) Shape_Dot(playerID, count, unit, 0, 0);
        Shape_Dot(playerID, count, unit, 0, 0)
        # (Line 424) }
        # (Line 425) else if (size > 1)
    if EUDElseIf()(size <= 1, neg=True):
        # (Line 426) {
        # (Line 427) foreach(idx : EUDLoopRange(loop))
        for idx in EUDLoopRange(loop):
            # (Line 428) {
            # (Line 429) if (size % 2 == 0)
            if EUDIf()(size % 2 == 0):
                # (Line 430) {
                # (Line 431) dis_x = idx % (size / 2) + 1;
                dis_x << (idx % (size // 2) + 1)
                # (Line 432) dis_y = idx / (size / 2) + 1;
                dis_y << (idx // (size // 2) + 1)
                # (Line 433) if (dis_x == 1)
                if EUDIf()(dis_x == 1):
                    # (Line 434) { Shape_Square(playerID, count, unit, space, dis_y * interval - space); }
                    Shape_Square(playerID, count, unit, space, dis_y * interval - space)
                    # (Line 435) else if (dis_y == 1)
                if EUDElseIf()(dis_y == 1):
                    # (Line 436) { Shape_Square(playerID, count, unit, dis_x * interval - space, space); }
                    Shape_Square(playerID, count, unit, dis_x * interval - space, space)
                    # (Line 437) else
                    # (Line 438) { Shape_Square(playerID, count, unit, dis_x * interval - space, dis_y * interval - space); }
                if EUDElse()():
                    Shape_Square(playerID, count, unit, dis_x * interval - space, dis_y * interval - space)
                    # (Line 439) }
                EUDEndIf()
                # (Line 440) else if (size % 2 == 1)
            if EUDElseIf()(size % 2 == 1):
                # (Line 441) {
                # (Line 442) dis_x = idx % (size / 2);
                dis_x << (idx % (size // 2))
                # (Line 443) dis_y = idx / (size / 2);
                dis_y << (idx // (size // 2))
                # (Line 445) if (idx == 0)
                if EUDIf()(idx == 0):
                    # (Line 446) { Shape_Dot(playerID, count, unit, 0, 0); }
                    Shape_Dot(playerID, count, unit, 0, 0)
                    # (Line 447) else if (dis_y == 0)
                if EUDElseIf()(dis_y == 0):
                    # (Line 448) { Shape_Square(playerID, count, unit, dis_x * interval, 0); }
                    Shape_Square(playerID, count, unit, dis_x * interval, 0)
                    # (Line 449) else
                    # (Line 450) { Shape_Square(playerID, count, unit, dis_x * interval + interval, dis_y * interval); }
                if EUDElse()():
                    Shape_Square(playerID, count, unit, dis_x * interval + interval, dis_y * interval)
                    # (Line 452) if (idx == loop - 1)
                EUDEndIf()
                if EUDIf()(idx == loop - 1):
                    # (Line 453) { Shape_Square(playerID, count, unit, (size / 2) * interval, 0); }
        ...
        # (Line 457) }
    EUDEndIf()
    # (Line 481) function Shape_Line(playerID : TrgPlayer, count, unit : TrgUnit, degree, size, interval, distance)

# (Line 482) {
@EUDTypedFunc([TrgPlayer, None, TrgUnit, None, None, None, None])
def Shape_Line(playerID, count, unit, degree, size, interval, distance):
    # (Line 483) Table_Sin(playerID, degree + 90, distance);
    Table_Sin(playerID, degree + 90, distance)
    # (Line 484) Table_Cos(playerID, degree + 90, distance);
    Table_Cos(playerID, degree + 90, distance)
    # (Line 486) var initial_x = v.P_AngleCos[playerID];
    initial_x = EUDVariable()
    initial_x << (v.P_AngleCos[playerID])
    # (Line 487) var initial_y = v.P_AngleSin[playerID];
    initial_y = EUDVariable()
    initial_y << (v.P_AngleSin[playerID])
    # (Line 489) if (size % 2 == 0)
    if EUDIf()(size % 2 == 0):
        # (Line 490) { Table_Sin(playerID, degree, interval * (size / 2) - interval / 2); Table_Cos(playerID, degree, interval * (size / 2) - interval / 2); }
        Table_Sin(playerID, degree, interval * (size // 2) - interval // 2)
        Table_Cos(playerID, degree, interval * (size // 2) - interval // 2)
        # (Line 491) else
        # (Line 492) { Table_Sin(playerID, degree, interval * (size / 2)); Table_Cos(playerID, degree, interval * (size / 2)); }
    if EUDElse()():
        Table_Sin(playerID, degree, interval * (size // 2))
        Table_Cos(playerID, degree, interval * (size // 2))
        # (Line 494) initial_x = initial_x + v.P_AngleCos[playerID];
    EUDEndIf()
    initial_x << (initial_x + v.P_AngleCos[playerID])
    # (Line 495) initial_y = initial_y + v.P_AngleSin[playerID];
    initial_y << (initial_y + v.P_AngleSin[playerID])
    # (Line 497) Table_Sin(playerID, degree, interval);
    Table_Sin(playerID, degree, interval)
    # (Line 498) Table_Cos(playerID, degree, interval);
    Table_Cos(playerID, degree, interval)
    # (Line 500) if (size == 1)
    if EUDIf()(size == 1):
        # (Line 501) { Shape_Dot(playerID, count, unit, 0, 0); }
        Shape_Dot(playerID, count, unit, 0, 0)
        # (Line 502) else if (size > 1)
    if EUDElseIf()(size <= 1, neg=True):
        # (Line 503) { foreach(idx : EUDLoopRange(size)) { Shape_Dot(playerID, count, unit, initial_x - idx * v.P_AngleCos[playerID], initial_y - idx * v.P_AngleSin[playerID]); } }
        ...
        # (Line 504) }
    EUDEndIf()
    # (Line 526) function Shape_Cross(playerID : TrgPlayer, count, unit : TrgUnit, degree, size, interval)

# (Line 527) {
@EUDTypedFunc([TrgPlayer, None, TrgUnit, None, None, None])
def Shape_Cross(playerID, count, unit, degree, size, interval):
    # (Line 528) var distance = interval / 2;
    distance = EUDVariable()
    distance << (interval // 2)
    # (Line 530) Table_Sin(playerID, degree, distance);
    Table_Sin(playerID, degree, distance)
    # (Line 531) Table_Cos(playerID, degree, distance);
    Table_Cos(playerID, degree, distance)
    # (Line 533) var gradient_x = v.P_AngleCos[playerID] / 2;
    gradient_x = EUDVariable()
    gradient_x << (v.P_AngleCos[playerID] // 2)
    # (Line 534) var gradient_y = v.P_AngleSin[playerID] / 2;
    gradient_y = EUDVariable()
    gradient_y << (v.P_AngleSin[playerID] // 2)
    # (Line 536) var angle_x, angle_y;
    angle_x, angle_y = EUDCreateVariables(2)
    # (Line 538) Table_Sin(playerID, degree, interval);
    Table_Sin(playerID, degree, interval)
    # (Line 539) Table_Cos(playerID, degree, interval);
    Table_Cos(playerID, degree, interval)
    # (Line 541) if (size == 1)
    if EUDIf()(size == 1):
        # (Line 542) {
        # (Line 543) Shape_Dot(playerID, count, unit, 0, 0);
        Shape_Dot(playerID, count, unit, 0, 0)
        # (Line 544) }
        # (Line 545) else if (size > 1)
    if EUDElseIf()(size <= 1, neg=True):
        # (Line 546) {
        # (Line 547) foreach(idx : EUDLoopRange(size))
        for idx in EUDLoopRange(size):
            # (Line 548) {
            # (Line 549) angle_x = v.P_AngleCos[playerID] * idx;
            angle_x << (v.P_AngleCos[playerID] * idx)
            # (Line 550) angle_y = v.P_AngleSin[playerID] * idx;
            angle_y << (v.P_AngleSin[playerID] * idx)
            # (Line 552) if (size % 2 == 0 && idx != size / 2)
            if EUDIf()(EUDSCAnd()(size % 2 == 0)(idx == size // 2, neg=True)()):
                # (Line 553) { Shape_Square(playerID, count, unit, angle_x + gradient_x, angle_y + gradient_y); }
                Shape_Square(playerID, count, unit, angle_x + gradient_x, angle_y + gradient_y)
                # (Line 554) else if (size % 2 == 1)
            if EUDElseIf()(size % 2 == 1):
                # (Line 555) {
                # (Line 556) if (idx == 0)
                if EUDIf()(idx == 0):
                    # (Line 557) { Shape_Dot(playerID, count, unit, 0, 0); }
                    Shape_Dot(playerID, count, unit, 0, 0)
                    # (Line 558) else
                    # (Line 559) { Shape_Square(playerID, count, unit, angle_x, angle_y); }
                if EUDElse()():
        ...
        # (Line 563) }
    EUDEndIf()
    # (Line 585) function Shape_Edge(playerID : TrgPlayer, count, unit : TrgUnit, degree, size, interval)

# (Line 586) {
@EUDTypedFunc([TrgPlayer, None, TrgUnit, None, None, None])
def Shape_Edge(playerID, count, unit, degree, size, interval):
    # (Line 587) Table_Sin(playerID, degree, interval * 14 / 10);
    Table_Sin(playerID, degree, interval * 14 // 10)
    # (Line 588) Table_Cos(playerID, degree, interval * 14 / 10);
    Table_Cos(playerID, degree, interval * 14 // 10)
    # (Line 590) var x_o = v.P_AngleCos[playerID];
    x_o = EUDVariable()
    x_o << (v.P_AngleCos[playerID])
    # (Line 591) var y_o = v.P_AngleSin[playerID];
    y_o = EUDVariable()
    y_o << (v.P_AngleSin[playerID])
    # (Line 593) var distance = (interval * 2) / (size - 1);
    distance = EUDVariable()
    distance << ((interval * 2) // (size - 1))
    # (Line 595) Table_Sin(playerID, degree + 45, distance);
    Table_Sin(playerID, degree + 45, distance)
    # (Line 596) Table_Cos(playerID, degree + 45, distance);
    Table_Cos(playerID, degree + 45, distance)
    # (Line 598) var distance_x = v.P_AngleCos[playerID];
    distance_x = EUDVariable()
    distance_x << (v.P_AngleCos[playerID])
    # (Line 599) var distance_y = v.P_AngleSin[playerID];
    distance_y = EUDVariable()
    distance_y << (v.P_AngleSin[playerID])
    # (Line 601) if (size == 1)
    if EUDIf()(size == 1):
        # (Line 602) { Shape_Dot(playerID, 1, unit, 0, 0); }
        Shape_Dot(playerID, 1, unit, 0, 0)
        # (Line 603) else if (size > 1)
    if EUDElseIf()(size <= 1, neg=True):
        # (Line 604) { foreach(idx : EUDLoopRange(size - 1)) { Shape_Square(playerID, 1, unit, x_o - (distance_x * idx), y_o - (distance_y * idx)); } }
