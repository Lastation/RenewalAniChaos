...
@EUDFunc
def f_main(playerID):
    # (Line 6) MoveLocation("23.Mayuri_Bozo", v.P_UnitID[playerID], playerID, "Anywhere");
    ...
    if EUDIf()(v.P_WaitMain[playerID] == 0):
        ...
        if EUDIf()(v.P_CountMain[playerID] == 0):
            # (Line 11) {
            # (Line 12) SetSwitch("Recall - Mayuri", Set);
            # (Line 14) if (playerID < 3) SetSwitch("Unique - Mayuri1", Set);
            DoActions(SetSwitch("Recall - Mayuri", Set))
            if EUDIf()(playerID >= 3, neg=True):
                # (Line 15) else SetSwitch("Unique - Mayuri2", Set);
                DoActions(SetSwitch("Unique - Mayuri1", Set))
            if EUDElse()():
                # (Line 17) SetDeaths(playerID, SetTo, 1080, " `UniqueCoolTime");
                DoActions(SetSwitch("Unique - Mayuri2", Set))
            EUDEndIf()
            # (Line 18) SetDeaths(playerID, SetTo, 180, " `UniqueSkill");
            ...
            # (Line 23) }
            # (Line 24) else if (v.P_CountMain[playerID] == 1)
        if EUDElseIf()(v.P_CountMain[playerID] == 1):
            # (Line 25) {
            # (Line 26) SetSwitch("Recall - Mayuri", Clear);
            # (Line 27) trg.SkillEnd();
