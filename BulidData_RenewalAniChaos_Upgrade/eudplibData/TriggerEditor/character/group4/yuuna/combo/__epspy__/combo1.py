## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY
from eudplib import *

def _RELIMP(path, mod_name):
    import inspect, pathlib, importlib.util
    p = pathlib.Path(inspect.getabsfile(inspect.currentframe())).parent
    for s in path.split("."):
        if s == "":  p = p.parent
        else:  p = p / s
    try:
        spec = importlib.util.spec_from_file_location(mod_name, p / (mod_name + ".py"))
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
    except FileNotFoundError:
        loader = EPSLoader(mod_name, str(p / (mod_name + ".eps")))
        spec = importlib.util.spec_from_loader(mod_name, loader)
        module = loader.create_module(spec)
        loader.exec_module(module)
    return module

def _IGVA(vList, exprListGen):
    def _():
        exprList = exprListGen()
        SetVariables(vList, exprList)
    EUDOnStart(_)

def _CGFW(exprf, retn):
    rets = [ExprProxy(None) for _ in range(retn)]
    def _():
        vals = exprf()
        for ret, val in zip(rets, vals):
            ret._value = val
    EUDOnStart(_)
    return rets

def _ARR(items):
    k = EUDArray(len(items))
    for i, item in enumerate(items):
        k[i] = item
    return k

def _VARR(items):
    k = EUDVArray(len(items))()
    for i, item in enumerate(items):
        k[i] = item
    return k

def _SRET(v, klist):
    return List2Assignable([v[k] for k in klist])

def _SV(dL, sL):
    [d << s for d, s in zip(FlattenList(dL), FlattenList(sL))]

class _ATTW:
    def __init__(self, obj, attrName):
        self.obj = obj
        self.attrName = attrName

    def __lshift__(self, r):
        setattr(self.obj, self.attrName, r)

    def __iadd__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov + v)

    def __isub__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov - v)

    def __imul__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov * v)

    def __ifloordiv__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov // v)

    def __iand__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov & v)

    def __ior__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov | v)

    def __ixor__(self, v):
        ov = getattr(self.obj, self.attrName)
        setattr(self.obj, self.attrName, ov ^ v)

class _ARRW:
    def __init__(self, obj, index):
        self.obj = obj
        self.index = index

    def __lshift__(self, r):
        self.obj[self.index] = r

    def __iadd__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov + v

    def __isub__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov - v

    def __imul__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov * v

    def __ifloordiv__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov // v

    def __iand__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov & v

    def __ior__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov | v

    def __ixor__(self, v):
        ov = self.obj[self.index]
        self.obj[self.index] = ov ^ v

def _L2V(l):
    ret = EUDVariable()
    if EUDIf()(l):
        ret << 1
    if EUDElse()():
        ret << 0
    EUDEndIf()
    return ret

def _MVAR(vs):
    return List2Assignable([
        v.makeL() if IsEUDVariable(v) else EUDVariable() << v
        for v in FlattenList(vs)])

def _LSH(l, r):
    if IsEUDVariable(l):  return f_bitlshift(l, r)
    else: return l << r

## NOTE: THIS FILE IS GENERATED BY EPSCRIPT! DO NOT MODITY

# (Line 1) import PluginVariables as msqcvar;
import PluginVariables as msqcvar
# (Line 2) import variable as v;
import variable as v
# (Line 3) import func.trig as trg;
from func import trig as trg
# (Line 5) function main(playerID)
# (Line 6) {
@EUDFunc
def f_main(playerID):
    # (Line 7) if (v.P_WaitMain[playerID] == 0)
    if EUDIf()(v.P_WaitMain[playerID] == 0):
        # (Line 8) {
        # (Line 9) if (v.P_CountMain[playerID] == 0)
        if EUDIf()(v.P_CountMain[playerID] == 0):
            # (Line 10) {
            # (Line 11) if (v.P_LoopMain[playerID] == 0)
            if EUDIf()(v.P_LoopMain[playerID] == 0):
                # (Line 12) {
                # (Line 13) trg.Shape_NxNSquare(playerID, 1, "40 + 1n Wraith", 3, 75);
                trg.Shape_NxNSquare(playerID, 1, "40 + 1n Wraith", 3, 75)
                # (Line 14) trg.Shape_NxNSquare(playerID, 1, " Unit. Hoffnung 25000", 3, 75);
                trg.Shape_NxNSquare(playerID, 1, " Unit. Hoffnung 25000", 3, 75)
                # (Line 15) KillUnitAt(All, "40 + 1n Wraith", "Anywhere", playerID);
                # (Line 16) KillUnitAt(All, " Unit. Hoffnung 25000", "Anywhere", playerID);
                DoActions(KillUnitAt(All, "40 + 1n Wraith", "Anywhere", playerID))
                # (Line 18) trg.Main_Wait(80);
                DoActions(KillUnitAt(All, " Unit. Hoffnung 25000", "Anywhere", playerID))
                trg.Main_Wait(80)
                # (Line 19) v.P_LoopMain[playerID] += 1;
                _ARRW(v.P_LoopMain, playerID).__iadd__(1)
                # (Line 20) }
                # (Line 21) else if (v.P_LoopMain[playerID] == 1)
            if EUDElseIf()(v.P_LoopMain[playerID] == 1):
                # (Line 22) {
                # (Line 23) trg.Shape_NxNSquare(playerID, 1, "40 + 1n Wraith", 5, 75);
                trg.Shape_NxNSquare(playerID, 1, "40 + 1n Wraith", 5, 75)
                # (Line 24) trg.Shape_NxNSquare(playerID, 1, " Unit. Hoffnung 25000", 5, 75);
                trg.Shape_NxNSquare(playerID, 1, " Unit. Hoffnung 25000", 5, 75)
                # (Line 25) KillUnitAt(All, "40 + 1n Wraith", "Anywhere", playerID);
                # (Line 26) KillUnitAt(All, " Unit. Hoffnung 25000", "Anywhere", playerID);
                DoActions(KillUnitAt(All, "40 + 1n Wraith", "Anywhere", playerID))
                # (Line 28) trg.Main_Wait(80);
                DoActions(KillUnitAt(All, " Unit. Hoffnung 25000", "Anywhere", playerID))
                trg.Main_Wait(80)
                # (Line 29) v.P_LoopMain[playerID] += 1;
                _ARRW(v.P_LoopMain, playerID).__iadd__(1)
                # (Line 30) }
                # (Line 31) else if (v.P_LoopMain[playerID] == 2)
            if EUDElseIf()(v.P_LoopMain[playerID] == 2):
                # (Line 32) {
                # (Line 33) trg.Shape_NxNSquare(playerID, 1, "50 + 1n Battlecruiser", 3, 75);
                trg.Shape_NxNSquare(playerID, 1, "50 + 1n Battlecruiser", 3, 75)
                # (Line 34) Order("50 + 1n Battlecruiser", playerID, "Anywhere", Attack, "Anywhere");
                # (Line 36) trg.Main_Wait(80);
                DoActions(Order("50 + 1n Battlecruiser", playerID, "Anywhere", Attack, "Anywhere"))
                trg.Main_Wait(80)
                # (Line 38) v.P_LoopMain[playerID] += 1;
                _ARRW(v.P_LoopMain, playerID).__iadd__(1)
                # (Line 39) }
                # (Line 40) else if (v.P_LoopMain[playerID] == 3)
            if EUDElseIf()(v.P_LoopMain[playerID] == 3):
                # (Line 41) {
                # (Line 42) trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 150);
                trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 150)
                # (Line 43) trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 75);
                trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 75)
                # (Line 44) trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 0);
                trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 0)
                # (Line 45) trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, -75);
                trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, -75)
                # (Line 46) trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, 150);
                trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, 150)
                # (Line 47) trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, 75);
                trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, 75)
                # (Line 48) trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, 0);
                trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, 0)
                # (Line 49) trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, -75);
                trg.Shape_Square(playerID, 1, "40 + 1n Guardian", 150, -75)
                # (Line 50) KillUnitAt(All, "40 + 1n Guardian", "Anywhere", playerID);
                # (Line 51) KillUnitAt(All, "Protoss Dark Archon", "Anywhere", playerID);
                DoActions(KillUnitAt(All, "40 + 1n Guardian", "Anywhere", playerID))
                # (Line 53) trg.Main_Wait(80);
                DoActions(KillUnitAt(All, "Protoss Dark Archon", "Anywhere", playerID))
                trg.Main_Wait(80)
                # (Line 55) v.P_CountMain[playerID] += 1;
                _ARRW(v.P_CountMain, playerID).__iadd__(1)
                # (Line 56) v.P_LoopMain[playerID] = 0;
                _ARRW(v.P_LoopMain, playerID) << (0)
                # (Line 57) }
                # (Line 58) }
            EUDEndIf()
            # (Line 59) else if (v.P_CountMain[playerID] == 1)
        if EUDElseIf()(v.P_CountMain[playerID] == 1):
            # (Line 60) {
            # (Line 61) if (v.P_LoopMain[playerID] < 4)
            if EUDIf()(v.P_LoopMain[playerID] >= 4, neg=True):
                # (Line 62) {
                # (Line 63) trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 150 - 75 * v.P_LoopMain[playerID]);
                trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 150, 150 - 75 * v.P_LoopMain[playerID])
                # (Line 64) KillUnitAt(All, "Protoss Dark Archon", "Anywhere", playerID);
                # (Line 66) trg.Main_Wait(80);
                DoActions(KillUnitAt(All, "Protoss Dark Archon", "Anywhere", playerID))
                trg.Main_Wait(80)
                # (Line 67) v.P_LoopMain[playerID] += 1;
                _ARRW(v.P_LoopMain, playerID).__iadd__(1)
                # (Line 68) }
                # (Line 69) else if (v.P_LoopMain[playerID] == 4)
            if EUDElseIf()(v.P_LoopMain[playerID] == 4):
                # (Line 70) {
                # (Line 71) KillUnitAt(All, "50 + 1n Battlecruiser", "Anywhere", playerID);
                # (Line 73) trg.Main_Wait(80);
                DoActions(KillUnitAt(All, "50 + 1n Battlecruiser", "Anywhere", playerID))
                trg.Main_Wait(80)
                # (Line 75) v.P_LoopMain[playerID] += 1;
                _ARRW(v.P_LoopMain, playerID).__iadd__(1)
                # (Line 76) }
                # (Line 77) else if (v.P_LoopMain[playerID] == 5)
            if EUDElseIf()(v.P_LoopMain[playerID] == 5):
                # (Line 78) {
                # (Line 79) trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, 150);
                trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, 150)
                # (Line 80) trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, 75);
                trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, 75)
                # (Line 81) trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, 0);
                trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, 0)
                # (Line 82) trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, -75);
                trg.Shape_Square(playerID, 1, "50 + 1n Battlecruiser", 150, -75)
                # (Line 83) trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, 150);
                trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, 150)
                # (Line 84) trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, 75);
                trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, 75)
                # (Line 85) trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, 0);
                trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, 0)
                # (Line 86) trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, -75);
                trg.Shape_Square(playerID, 1, "50 + 1n Tank", 150, -75)
                # (Line 87) KillUnitAt(All, "50 + 1n Battlecruiser", "Anywhere", playerID);
                # (Line 88) KillUnitAt(All, "50 + 1n Tank", "Anywhere", playerID);
                DoActions(KillUnitAt(All, "50 + 1n Battlecruiser", "Anywhere", playerID))
                # (Line 90) trg.Main_Wait(80);
                DoActions(KillUnitAt(All, "50 + 1n Tank", "Anywhere", playerID))
                trg.Main_Wait(80)
                # (Line 92) v.P_CountMain[playerID] += 1;
                _ARRW(v.P_CountMain, playerID).__iadd__(1)
                # (Line 93) v.P_LoopMain[playerID] = 0;
                _ARRW(v.P_LoopMain, playerID) << (0)
                # (Line 94) }
                # (Line 95) }
            EUDEndIf()
            # (Line 98) else if (v.P_CountMain[playerID] == 2)
        if EUDElseIf()(v.P_CountMain[playerID] == 2):
            # (Line 99) {
            # (Line 100) if (v.P_LoopMain[playerID] == 0)
            if EUDIf()(v.P_LoopMain[playerID] == 0):
                # (Line 101) {
                # (Line 102) trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, 100);
                trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, 100)
                # (Line 103) trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, 50);
                trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, 50)
                # (Line 104) trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, 0);
                trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, 0)
                # (Line 105) trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, -50);
                trg.Shape_Square(playerID, 1, "40 + 1n Mojo", 100, -50)
                # (Line 106) trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, 100);
                trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, 100)
                # (Line 107) trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, 50);
                trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, 50)
                # (Line 108) trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, 0);
                trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, 0)
                # (Line 109) trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, -50);
                trg.Shape_Square(playerID, 1, "60 + 1n Archon", 100, -50)
                # (Line 110) KillUnitAt(All, "60 + 1n Archon", "Anywhere", playerID);
                # (Line 111) MoveLocation(v.P_LocationID[playerID], v.P_UnitID[playerID], playerID, "Anywhere");
                DoActions(KillUnitAt(All, "60 + 1n Archon", "Anywhere", playerID))
                # (Line 112) Order("40 + 1n Mojo", playerID, "Anywhere", Attack, v.P_LocationID[playerID]);
                DoActions(MoveLocation(v.P_LocationID[playerID], v.P_UnitID[playerID], playerID, "Anywhere"))
                # (Line 114) trg.Main_Wait(160);
                DoActions(Order("40 + 1n Mojo", playerID, "Anywhere", Attack, v.P_LocationID[playerID]))
                trg.Main_Wait(160)
                # (Line 116) v.P_LoopMain[playerID] += 1;
                _ARRW(v.P_LoopMain, playerID).__iadd__(1)
                # (Line 117) }
                # (Line 118) else if (v.P_LoopMain[playerID] == 1)
            if EUDElseIf()(v.P_LoopMain[playerID] == 1):
                # (Line 119) {
                # (Line 120) KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID);
                # (Line 122) trg.Shape_NxNSquare(playerID, 1, "40 + 1n Mojo", 3, 50);
                DoActions(KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID))
                trg.Shape_NxNSquare(playerID, 1, "40 + 1n Mojo", 3, 50)
                # (Line 123) trg.Shape_NxNSquare(playerID, 1, "60 + 1n Archon", 3, 50);
                trg.Shape_NxNSquare(playerID, 1, "60 + 1n Archon", 3, 50)
                # (Line 124) KillUnitAt(All, "60 + 1n Archon", "Anywhere", playerID);
                # (Line 125) KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID);
                DoActions(KillUnitAt(All, "60 + 1n Archon", "Anywhere", playerID))
                # (Line 127) trg.Main_Wait(160);
                DoActions(KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID))
                trg.Main_Wait(160)
                # (Line 129) v.P_CountMain[playerID] += 1;
                _ARRW(v.P_CountMain, playerID).__iadd__(1)
                # (Line 130) v.P_LoopMain[playerID] = 0;
                _ARRW(v.P_LoopMain, playerID) << (0)
                # (Line 131) }
                # (Line 132) }
            EUDEndIf()
            # (Line 133) else if (v.P_CountMain[playerID] == 3)
        if EUDElseIf()(v.P_CountMain[playerID] == 3):
            # (Line 134) {
            # (Line 135) KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID);
            # (Line 137) trg.SkillEnd();
            DoActions(KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID))
            trg.SkillEnd()
            # (Line 138) }
            # (Line 139) }
        EUDEndIf()
        # (Line 140) }
    EUDEndIf()
