...
def f_main(playerID):
    ...
    if EUDIf()(v.P_WaitMain[playerID] == 0):
        ...
        _t10 = EUDElseIf()
        # (Line 100) && Bring(playerID, AtLeast, 2, "Protoss Carrier", "[Skill]UseSkill"))
        if _t10(EUDSCAnd()(v.P_CountMain[playerID] == 4)(Bring(playerID, AtLeast, 2, "Protoss Carrier", "[Skill]UseSkill"))()):
            # (Line 101) {
            # (Line 102) KillUnitAt(All, "40 + 1n Marine", "Anywhere", playerID);
            # (Line 103) KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID);
            DoActions(KillUnitAt(All, "40 + 1n Marine", "Anywhere", playerID))
            # (Line 104) KillUnitAt(All, "50 + 1n Tank", "Anywhere", playerID);
            DoActions(KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID))
            # (Line 105) KillUnitAt(All, "60 + 1n Danimoth", "Anywhere", playerID);
            DoActions(KillUnitAt(All, "50 + 1n Tank", "Anywhere", playerID))
            # (Line 107) s.CharacterVoice(3);
            DoActions(KillUnitAt(All, "60 + 1n Danimoth", "Anywhere", playerID))
            s.CharacterVoice(3)
            # (Line 108) KillUnitAt(2, "Protoss Carrier", "[Skill]UseSkill", playerID);
            # (Line 110) v.P_Step[playerID] = 220;
            ...
            _ARRW(v.P_LoopMain, playerID) << (0)
            # (Line 113) }
            # (Line 114) else if (v.P_CountMain[playerID] == 4)
        if EUDElseIf()(v.P_CountMain[playerID] == 4):
            # (Line 115) {
            # (Line 116) KillUnitAt(All, "40 + 1n Marine", "Anywhere", playerID);
            # (Line 117) KillUnitAt(All, "40 + 1n Mojo", "Anywhere", playerID);
            DoActions(KillUnitAt(All, "40 + 1n Marine", "Anywhere", playerID))
