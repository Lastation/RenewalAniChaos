...
def f_main(playerID):
    ...
    if EUDIf()(v.P_WaitMain[playerID] == 0):
        ...
        _t26 = EUDElseIf()
        # (Line 211) && Bring(playerID, AtLeast, 2, "Protoss Arbiter", "[Skill]UseSkill")
        # (Line 212) && Accumulate(playerID, AtLeast, 600, Gas))
        if _t26(EUDSCAnd()(v.P_CountMain[playerID] == 9)(Bring(playerID, AtLeast, 2, "Protoss Arbiter", "[Skill]UseSkill"))(Accumulate(playerID, AtLeast, 600, Gas))()):
            # (Line 213) {
            ...
            s.CharacterVoice(5)
            # (Line 219) KillUnitAt(2, "Protoss Arbiter", "[Skill]UseSkill", playerID);
            # (Line 221) v.P_Step[playerID] = 230;
            DoActions(KillUnitAt(2, "Protoss Arbiter", "[Skill]UseSkill", playerID))
            _ARRW(v.P_Step, playerID) << (230)
            # (Line 222) SetResources(playerID, Subtract, 600, Gas);
            # (Line 223) v.P_CountMain[playerID] = 0;
            DoActions(SetResources(playerID, Subtract, 600, Gas))
            _ARRW(v.P_CountMain, playerID) << (0)
            # (Line 224) v.P_LoopMain[playerID] = 0;
            _ARRW(v.P_LoopMain, playerID) << (0)
            # (Line 225) }
            # (Line 226) else if (v.P_CountMain[playerID] == 9)
        if EUDElseIf()(v.P_CountMain[playerID] == 9):
            # (Line 227) {
            # (Line 228) KillUnitAt(All, "40 + 1n Marine", "Anywhere", playerID);
