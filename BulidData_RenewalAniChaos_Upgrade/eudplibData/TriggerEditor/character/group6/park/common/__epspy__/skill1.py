...
def f_main(playerID):
    ...
    if EUDIf()(v.P_WaitMain[playerID] == 0):
        ...
        if EUDIf()(v.P_CountMain[playerID] == 0):
            # (Line 10) {
            # (Line 11) if (v.P_LoopMain[playerID] < 12)
            if EUDIf()(v.P_LoopMain[playerID] >= 12, neg=True):
                # (Line 12) {
                # (Line 13) trg.Shape_Square(playerID, 1, "Bengalaas (Jungle)", 32, 32);
        ...
        if EUDElseIf()(v.P_CountMain[playerID] == 1):
            # (Line 28) {
            # (Line 29) if (v.P_LoopMain[playerID] < 12 )
            if EUDIf()(v.P_LoopMain[playerID] >= 12, neg=True):
                # (Line 30) {
                # (Line 31) trg.Shape_Square(playerID, 1, "Bengalaas (Jungle)", 64, 64);
                trg.Shape_Square(playerID, 1, "Bengalaas (Jungle)", 64, 64)
                # (Line 32) trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 64, 0);
                trg.Shape_Square(playerID, 1, "Protoss Dark Archon", 64, 0)
                # (Line 33) KillUnitAt(All, "Bengalaas (Jungle)", "Anywhere", playerID);
        ...
        if EUDElseIf()(v.P_CountMain[playerID] == 2):
            ...
            if EUDIf()(v.P_LoopMain[playerID] == 8):
                # (Line 51) {
                # (Line 52) trg.Shape_NxNSquare(playerID, 1, "50 + 1n Battlecruiser", 3, 100);
                trg.Shape_NxNSquare(playerID, 1, "50 + 1n Battlecruiser", 3, 100)
                # (Line 53) trg.Shape_NxNSquare(playerID, 1, "Protoss Dark Archon", 3, 50);
            ...
            _ARRW(v.P_LoopMain, playerID).__iadd__(1)
            # (Line 61) if (v.P_LoopMain[playerID] == 10)
            if EUDIf()(v.P_LoopMain[playerID] == 10):
                ...
                # (Line 66) }
            EUDEndIf()
            # (Line 68) else if (v.P_CountMain[playerID] == 3)
        if EUDElseIf()(v.P_CountMain[playerID] == 3):
            # (Line 69) {
            # (Line 70) SetDeaths(playerID, SetTo, 1200, " `UniqueCoolTime");
            # (Line 71) SetDeaths(playerID, SetTo, 120, " `UniqueSkill");
