...
{
   if (Deaths(CurrentPlayer, AtLeast, 1054, " `UniqueCoolTime"))
   {
      if (Deaths(CurrentPlayer, Exactly, 1080, " `UniqueCoolTime"))
      {
         trg.Shape_NxNSquare(playerID, 1, "40 + 1n Guardian", 3, 50);
         KillUnitAt(All, "40 + 1n Guardian", "Anywhere", playerID);
      }
      else if (Deaths(CurrentPlayer, Exactly, 1055, " `UniqueCoolTime"))
      {
